...
function ContactPageInner() {
  ...
  // Update document title client-side for B2B variant (since metadata is static)
  useEffect(() => {
    if (isB2B) {
      document.title = "פנייה ארגונית | אלעד יעקובוביץ' - מפתח Full-Stack";
    }
  }, [isB2B]);
  ...
  const buildFinalMessage = (): string => {
    ...
    const lines = [
      header,
      `ארגון: ${cap(b2bData.organization) || '-'}`,
      `תפקיד: ${cap(b2bData.role) || '-'}`,
      `גודל: ${cap(b2bData.orgSize, 50) || '-'}`,
      `תקציב: ${cap(b2bData.budget, 50) || '-'}`,
      `סוג פרויקט: ${cap(b2bData.projectType, 100) || '-'}`,
      '',
      formData.message,
    ];
    return lines.join('\n');
                  ...
                  >
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6" noValidate>
                      ...
                      {isB2B && (
                        ...
                        >
                          {/*
                           * Single helper paragraph linked from every B2B field via
                           * aria-describedby. Screen readers will read each field's
                           * label, then this helper. All B2B fields are optional
                           * per spec; we don't add aria-required.
                           */}
                          <p
                            id="b2b-helper"
