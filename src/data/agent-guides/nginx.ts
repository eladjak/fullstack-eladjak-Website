...
export const nginxGuide: AgentGuideData = {
  ...
  stats: [
    { label: "נתח מאתרי העולם", value: "~33%" },
  ...
  paradigmShifts: [
    {
      ...
      after: "nginx מקבל את הכול ומנתב פנימה ב-HTTP פשוט",
      icon: Lock,
    },
    {
  ...
  sections: [
    {
      ...
      difficulty: "beginner",
      beginner:
        "תחשבו על זה כמו על פקיד הקבלה במשרד גדול: כל מי שנכנס מהדלת מגיע אליו קודם, והוא זה שמחליט לאיזה חדר להפנות אותו. הוא לא עושה את העבודה עצמה, הוא רק מנתב. בלי הפקיד הזה, כל אורח היה צריך לדעת איפה כל אחד יושב, והיה כאוס. nginx הוא הפקיד של האינטרנט.",
      content: [
        ...
        "Proxy pass, הפקודה הקסומה: `proxy_pass http://localhost:3001`, שולחת את הבקשה לשירות הפנימי ומחזירה את התשובה למשתמש",
        "Static files, nginx מצוין בהגשת קבצים מדיסק (HTML, CSS, JS, תמונות). מהיר פי 100 מ-Node או Python",
        "Worker processes, nginx מריץ כמה תהליכים מקבילים, אחד לכל ליבת CPU. כך הוא מנצל את כל החומרה ביעילות",
      ],
      tips: [
        "ב-Ubuntu/Debian: `apt install nginx`, מתקין כשירות שעולה אוטומטית בכל אתחול",
        "הקבצים נמצאים ב-`/etc/nginx/`. הקובץ הראשי `nginx.conf`, והאתרים ב-`sites-available/` עם symlink ל-`sites-enabled/`",
    ...
    },
    {
      ...
      description:
        "Reverse proxy הוא השם הטכני לתפקיד הקלאסי של nginx ב-2026: לעמוד מול האינטרנט, לקבל את כל הבקשות, ולנתב אותן ל-backend הפנימי המתאים. 'הפוך' (reverse) כי שלא כמו proxy רגיל ש'מסתיר' את הלקוח (כמו VPN), reverse proxy מסתיר את השרתים, הלקוח חושב שהוא מדבר עם שרת אחד, אבל בפועל יש מאחוריו עשרות. בשרתים שהקמתי בעבר, nginx קיבל בקשות לעשרות תת-דומיינים שונים וניתב כל אחת לקונטיינר Docker אחר שרץ על פורט פנימי.",
      color: "from-emerald-600 to-teal-500",
      difficulty: "intermediate",
      beginner:
        "זה השימוש שבגללו רוב האנשים מתקינים nginx, וקל להבין אותו עם דימוי: דמיינו מרכזייה טלפונית של חברה גדולה. מתקשרים למספר אחד, ופקיד המרכזייה מעביר אתכם למחלקה הנכונה, מבלי שתדעו שיש בכלל מאחור עשרות שלוחות פנימיות. nginx עושה בדיוק את זה לאתרים: הגולש פונה לכתובת אחת, ו-nginx מעביר אותו בשקט לשירות הפנימי המתאים. ככה עשרות תת-דומיינים שונים יכולים לחיות על שרת אחד, כשכל אחד מגיע לקונטיינר אחר, בדיוק כמו שעשיתי בשרתים שניהלתי בעבר.",
      content: [
        "proxy_pass, הפקודה המרכזית. `proxy_pass http://localhost:3001/`, לוקחת את הבקשה ושולחת אותה לשירות הפנימי",
        ...
        "Buffering, nginx כברירת מחדל אוסף את כל התשובה לפני ששולח אותה. ל-streaming (SSE, video) צריך `proxy_buffering off`",
        "Timeouts, `proxy_read_timeout` חשוב לבקשות ארוכות (LLM, video processing). ברירת המחדל היא 60 שניות, לעיתים צריך הרבה יותר",
        "Path-based routing, אותו דומיין יכול לנתב לפי נתיב: `/api/v1` → service A, `/api/v2` → service B, `/` → frontend",
      ],
      tips: [
        "תמיד הוסיפו סלאש בסוף proxy_pass: `proxy_pass http://x:3001/` (עם /) שונה מ-`proxy_pass http://x:3001` (בלי). ההבדל הוא איך הנתיב מועבר. קראו את התיעוד פעם אחת והדפיסו",
        "לקבצי upload גדולים, הוסיפו `client_max_body_size 100M;` בתוך ה-server block. ברירת המחדל היא 1M, קטן מדי לרוב המקרים",
        "מבנה מומלץ: כל קובץ הגדרה לשירות יושב ב-`/etc/nginx/sites-available/<service>.conf`, קל לערוך, לראות diff ב-git, ולהפעיל/לכבות בנפרד",
      ],
      codeExample: {
    ...
    },
    {
      ...
      subtitle: "תעודה חינמית, מתחדשת אוטומטית, על כל דומיין",
      description:
      ...
      difficulty: "intermediate",
      beginner:
        "SSL זה מה שעושה את המנעול הקטן הירוק שאתם רואים ליד הכתובת בדפדפן, הוא מצפין את כל מה שעובר בין הגולש לאתר, כך שאף אחד באמצע (בית קפה, ספק אינטרנט, האקר) לא יכול לקרוא או לשנות אותו. פעם תעודה כזאת עלתה כסף והייתה כאב ראש להתקין; היום, בזכות שירות חינמי בשם Let's Encrypt, הכול קורה אוטומטית, מקבלים תעודה תוך דקה, והיא אפילו מתחדשת לבד כל חודשיים בלי שתיגעו. אין שום סיבה ב-2026 לאתר בלי מנעול.",
      content: [
        ...
        "גרסאות TLS, כבו את 1.0 ו-1.1 (פגיעות). הגדירו `ssl_protocols TLSv1.2 TLSv1.3`",
        "HSTS, header שאומר לדפדפן 'תמיד השתמש ב-HTTPS לדומיין הזה'. הוסיפו `add_header Strict-Transport-Security 'max-age=31536000; includeSubDomains' always;`",
        "OCSP stapling, מאיץ את ה-handshake של ה-SSL. `ssl_stapling on; ssl_stapling_verify on;`",
      ],
      tips: [
        "Mozilla מספקים גנרטור לתצורת SSL: ssl-config.mozilla.org, תבחרו את גרסת ה-nginx, ותקבלו תצורה אופטימלית ומאובטחת",
        "אל תשכחו לפתוח את פורט 80 ב-firewall גם אחרי שעברתם ל-HTTPS, Let's Encrypt צריכים אותו לאימות ההתחדשות",
    ...
    },
    {
      ...
      difficulty: "advanced",
      beginner:
        "כאן nginx מפסיק להיות רק 'פקיד מנתב' והופך גם לשומר ולחוסך כסף. שלושת הדברים בסעיף שווים בעברית פשוטה: rate limiting זה מאבטח שלא נותן לאותו אדם לדפוק על הדלת אלף פעם בשנייה (הגנה מבוטים והצפות). caching זה לזכור תשובה שכבר חישבנו, במקום להטריח את האפליקציה לחשב אותה שוב ושוב לכל גולש. ו-load balancing זה לחלק את העומס בין כמה עותקים של האפליקציה, כמו לפתוח עוד קופות בסופר כשהתור מתארך. כל אחד מהם יכול לחסוך לכם הרבה כסף וכאב ראש בייצור.",
      content: [
        ...
        "Connection limiting, `limit_conn_zone` מגביל את מספר החיבורים הפתוחים מאותו IP. שימושי נגד slow-loris attack",
        ...
        "Load balancing, `upstream` מגדיר קבוצת שרתים. ברירת המחדל היא round-robin; אפשר `least_conn` (שולח לשרת עם פחות חיבורים) או `ip_hash` (תמיד אותו IP לאותו שרת, sticky sessions)",
      ...
      tips: [
        ...
        "Cache של API responses הוא החלום, אבל ודאו שאתם מטפלים נכון ב-cache invalidation. כלל הברזל: בדקו אם המידע מתעדכן או לא, ורק אז החליטו",
    ...
    },
    {
      id: "alternatives",
      icon: GitBranch,
      title: "Caddy, Traefik, ומתי לבחור מה",
      subtitle: "nginx הוא הסטנדרט, אבל לא תמיד הבחירה הנכונה",
      description:
        "ב-2026 יש שלוש בחירות מרכזיות ל-reverse proxy על שרת אישי: nginx (הוותיק, הסטנדרט), Caddy (החדש, פשוט בטירוף), ו-Traefik (מובנה ל-Docker). כל אחד מהם יעבוד טוב, השאלה היא איזה סיבוך אתם מוכנים לשלם תמורת איזה כוח. אצלי (אלעד), השרתים הראשונים היו על nginx; היום הקצה שלי בנוי מ-Traefik יחד עם [Cloudflare Tunnel](/guide/cloudflare-tunnel). אבל nginx עדיין הסטנדרט בתעשייה ותפגשו אותו אצל לקוחות.",
      color: "from-rose-600 to-pink-500",
      difficulty: "intermediate",
      beginner:
        "nginx הוא הוותיק והסטנדרט, אבל הוא לא היחיד, ויש מקרים שבהם משהו אחר פשוט נוח יותר. Caddy, למשל, עושה את אותה עבודה אבל בשורה אחת של הגדרה ודואג למנעול הירוק לבד, בלי שום צעד נוסף; Traefik 'נדבק' אוטומטית לכל מה שרץ ב-Docker. הכלל שלי (אלעד) פשוט: לפרויקט אישי חדש שרוצים פשטות, תתחילו מ-Caddy. אבל אם אתם בונים משהו שיגיע לייצור רציני או ללקוחות, שווה ללמוד nginx, כי אותו תפגשו בכל מקום.",
      content: [
        "Caddy, קובץ ההגדרה נקרא Caddyfile, בפורמט פשוט מאוד. שורה אחת = SSL מ-Let's Encrypt אוטומטית, בלי certbot, בלי הגדרה. מצוין ל-side projects ולפרויקטים אישיים",
        "Traefik, מתחבר אוטומטית ל-Docker labels. מוסיפים `traefik.http.routers.x.rule=Host('x.com')` לקונטיינר, ו-Traefik מנתב אליו אוטומטית. מצוין ל-Docker-only setups",
        "nginx, היכולת הגדולה ביותר, התיעוד הגדול ביותר, התמיכה בכל מקום. אבל צריך ללמוד יותר, והתעודות דורשות certbot נפרד",
        "ביצועים, שלושתם דומים בעומס תקין. nginx מנצח קצת בעומס קיצוני, Caddy שני, Traefik שלישי. ברוב המקרים ההבדל לא משמעותי",
        "OpenResty, הרחבה של nginx עם Lua מובנה. מאפשרת לוגיקה מתקדמת ב-config. למי שצריך API gateway רציני",
        "HAProxy, אופציה רביעית, מתמחה ב-load balancing TCP/HTTP. מהיר ויציב במיוחד, אבל פחות נפוץ ל-reverse proxy של HTTP פשוט",
      ],
      tips: [
        "אם אתם מתחילים פרויקט חדש ב-2026 ורוצים פשטות, התחילו מ-Caddy. אם אתם בונים משהו שיגיע לייצור רציני או ללקוחות, למדו nginx",
        "מעבר מ-nginx ל-Caddy יכול לחסוך כ-70% מהקוד. במקום 50 שורות nginx config לכל שירות, 3 שורות Caddyfile",
    ...
    },
    {
      ...
      difficulty: "intermediate",
      beginner:
        "כשמשהו ב-nginx לא עובד, אל תנחשו, תסתכלו. שני כלים פותרים כמעט הכול. הראשון הוא `nginx -t`, פקודה ש'מגיהה' את ההגדרות שלכם ואומרת אם יש בהן טעות כתיב לפני שמפעילים אותן, כמו בודק איות לפני ששולחים מסמך. השני הוא קובץ ה-error.log, היומן שמספר בדיוק מה השתבש ולמה. רוב התקלות הן דבר קטן וטיפשי (סלאש שנשכח, פורט לא נכון), ושני הכלים האלה חושפים אותו תוך דקות במקום שעות של ניחושים.",
      content: [
        "`nginx -t`, בודק תקינות תחבירית של כל קבצי ה-config. תמיד הריצו לפני reload. אם זה לא עובר, nginx ימשיך עם התצורה הישנה",
        ...
        "`/var/log/nginx/error.log`, שגיאות. אם משהו לא עובד, פה תמצאו את הסיבה. שגיאות SSL, timeouts, upstream down, הכול פה",
        "`curl -I https://example.com`, בודק את ה-headers ש-nginx מחזיר. רואים סטטוס, redirects ושמירה במטמון",
        "`openssl s_client -connect example.com:443 -servername example.com`, בודק את תעודת ה-SSL: מי הוציא אותה, מתי פגה, אילו פרוטוקולים נתמכים",
        "ssllabs.com, בודק את האבטחה של ה-SSL שלכם. המטרה: A או A+",
      ],
      tips: [
        "כשמשהו לא עובד, צעד 1 תמיד: `tail -f /var/log/nginx/error.log`, ואז פתחו את הדף בדפדפן. השגיאה תופיע מיד",
        "ב-nginx, הסדר של ה-directives חשוב, `location` ספציפי יותר קודם, פחות ספציפי אחר כך. כשמשהו 'מוזר' קורה, ודאו שה-location הנכון מטופל",
        "הוסיפו לכל server block: `add_header X-Served-By 'nginx-prod' always;`. כך, כשאתם מנסים לפתור בעיה, רואים מיד מאיזה שרת הגיעה התשובה",
      ],
    },
  ],
  resources: [
    ...
    },
    {
      ...
      description: "המדריכים הטובים ביותר באנגלית קלה לכל נושא",
    ...
    },
    {
      ...
      description: "אלטרנטיבה ל-nginx + פתיחת פורטים, הכול בלי לחשוף את ה-VPS",
  ...
  authorBio:
    "בשרתים הראשונים שהקמתי, nginx ניתב עשרות תת-דומיינים לקונטיינרים פנימיים, טיפל ב-SSL מ-Let's Encrypt, ורץ חודשים ברצף בלי תחזוקה. היום הקצה של רשת הסוכנים שלי בנוי מ-Cloudflare Tunnel ו-Traefik, והאתר הזה רץ על Vercel, אבל nginx נשאר הסטנדרט שאני ממליץ ללמוד, כי תפגשו אותו כמעט אצל כל לקוח. המדריך הזה מבוסס על שנים של setupים ואיתורי תקלות.",
};
