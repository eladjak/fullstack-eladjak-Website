...
export const hermesGuide: AgentGuideData = {
  ...
  stats: [
    ...
    { label: "ניסיונות לפני escalation", value: "3" },
  ...
  paradigmShifts: [
    {
      ...
      after: "Hermes ניסה restart, זה עבד, ובבוקר נשלח email 'היה תקלה ותוקן'",
      icon: RefreshCw,
    },
    {
      before: "להריץ את אותו script תיקון בפעם החמישית השבוע",
      after: "Hermes שומר 'מה עבד על מה' ומיישם אוטומטית",
      icon: Lightbulb,
    },
    {
      before: "PagerDuty, BetterStack, Grafana OnCall — 21-100+ דולר לחודש למשתמש",
      after: "Hermes פתוח, ציבורי, וחוקי תיקון ב-JSON",
      icon: Shield,
    },
    {
  ...
  sections: [
    {
      ...
      difficulty: "beginner",
      beginner:
        "חשבו על זה ככה: במקום שה-on-call שלכם ישלם 100 דולר בחודש ל-PagerDuty, ויתעורר ב-03:00 כדי לעשות `docker restart` למה שנופל כל שלישי בלילה — Hermes הוא כמו מטפל שמתקן את עצמו. הוא יושב על השרת, בודק כל 5 דקות שהכל חי, וכשמשהו נשבר הוא עושה את הדבר ההגיוני הראשון (למשל restart), בודק שזה באמת עבד, ורושם לעצמו 'זה עבד — בפעם הבאה שזה קורה, אני יודע מה לעשות'. רק אם שלושה ניסיונות נכשלו הוא שולח לכם התראה — ואז אתם יודעים שזה באמת משהו חדש ומעניין, לא עוד תקלה משעממת.",
      content: [
        "Detect — ניטור רציף של השירותים ([cron](/guide/dashboard) שרץ כל 5 דקות ומריץ healthcheck על כל מה שחשוב: docker ps, journalctl, בקשת /health על כל endpoint — המקבילה ל-SRE בגוגל, בקטן)",
        ...
        "Escalate — רק אחרי 3 ניסיונות כושלים, או אם 2+ שירותים נופלים במקביל (cascade) — נשלחת התראה אנושית עם runbook מצורף: 'ניסיתי X, Y, Z, זה מה שה-log אומר'",
      ],
      tips: [
        "אל תתחילו עם CLI חיצוני מסובך — הקוד הראשוני של Hermes היה 80 שורות Python שרצו ב-[cron](/guide/dashboard). רק אחרי שה-pattern הוכיח את עצמו עברתי ל-Go",
