...
export const vercelGuide: AgentGuideData = {
  slug: "vercel",
  agentName: "Vercel",
  agentNameHe: "Vercel — פלטפורמה ל-deploy של Next.js בדקה וחצי",
  category: "infra",
  brandIconSlug: "vercel",
  brandIconColor: "FFFFFF",
  heroBgImage: "/images/guides/guide-vercel-hero.jpg",
  tagline: "git push → אתר חי. CDN גלובלי, SSL אוטומטי, preview deployments. ה-tier החינמי נדיב.",
  heroDescription:
  ...
  paradigmSub:
    "אין yaml, אין dockerfile, אין SSH. git push הוא ה-deploy. הפלטפורמה עושה את השאר.",
  paradigmShifts: [
    {
    ...
    },
    {
      ...
      after: "Vercel Dashboard מציג את הכול בזמן אמת",
      icon: Activity,
    },
    {
  ...
  whoIsThisFor: [
    {
      title: "מפתחי Next.js",
      description:
        "Vercel נבנתה ספציפית ל-Next.js. תמיכה ב-ISR, server actions, edge functions — הכול עובד מהקופסה.",
      icon: Rocket,
    ...
    },
    {
      ...
      description:
        "אם אתם בונים frontend בלבד, Vercel חוסך 100% מעבודת התשתית. תהיו פנויים להתמקד ב-UX.",
      icon: Cloud,
  ...
  sections: [
    {
      ...
      subtitle: "פלטפורמה ל-frontend, לא ענן כללי",
      description:
        "Vercel היא PaaS (Platform as a Service) ספציפית לאתרי web frontend ו-serverless functions. שלא כמו AWS/GCP/Azure שמציעות אינסוף שירותים גנריים (VMs, databases, ML, וכו'), Vercel מתמקדת בדבר אחד: לפרוס אתרים מודרניים בכמה שניות. החברה נוסדה ב-2015 (בשם 'ZEIT', שונה ל-Vercel ב-2020) על ידי הצוות שיצר את Next.js — ולכן יש לה התמיכה הטובה בעולם ל-framework הזה. אבל היא תומכת גם בכל framework של frontend, כולל אתרים סטטיים פשוטים ב-HTML.",
      color: "from-blue-600 to-cyan-500",
      difficulty: "beginner",
      beginner:
        "תחשבו על Vercel כמו על Netlify (אבל מהיר יותר ועם יותר פיצ'רים) או GitHub Pages (אבל עם backend ו-server-side rendering). אתם נותנים להם את הקוד — הם נותנים לכם אתר חי עם דומיין, SSL ו-CDN. אין שרתים, אין SSH, אין /var/log.",
      content: [
        "Build & Deploy אוטומטי — מחברים GitHub/GitLab/Bitbucket repo, ו-Vercel מתחיל לבנות אוטומטית בכל push",
        "Preview deployments — כל branch ו-PR מקבלים URL ייחודי. למשל `myapp-feature-x.vercel.app`",
        "Edge Network — האתר מוגש מ-100+ אזורים גלובליים. latency נמוך מכל מקום בעולם",
        "Serverless Functions — `/api/*` בפרויקט שלכם הופך אוטומטית ל-AWS Lambda. אין צורך להגדיר",
      ...
      tips: [
        "Vercel חינמי לפרויקטים אישיים ו-hobby. ברגע שאתם מרוויחים מהאתר ('commercial'), טכנית צריך לעבור ל-Pro ($20/חודש)",
    ...
    },
    {
      ...
      content: [
        ...
        "5. Add Environment Variables — אם יש לכם API keys, הוסיפו אותם כאן (לא בקוד!)",
        ...
        "8. אוטומטית — מהיום, כל push ל-main עושה deploy אוטומטי",
      ],
      tips: [
        "הוסיפו ל-`.gitignore` את `.vercel/` (נוצר רק אם מריצים `vercel CLI` מקומית)",
        "Environment variables אפשר להגדיר ל-3 רמות: Production, Preview, Development. שימושי כדי לתת מפתחות שונים לסביבות שונות",
        "אם ה-build נכשל, פתחו את ה-deploy ב-Vercel Dashboard — הלוגים שם מאוד ברורים, רואים בדיוק את שורת ה-error",
      ],
      codeExample: {
        label: "Vercel CLI ל-deploy משורת הפקודה",
        code: "# התקנה גלובלית\nnpm i -g vercel\n\n# בתוך תיקיית הפרויקט\nvercel login\nvercel link  # מחבר את התיקייה לפרויקט קיים\n\n# Deploy ל-preview (URL ייחודי)\nvercel\n\n# Deploy ל-production\nvercel --prod\n\n# צפייה ב-logs בזמן אמת של production\nvercel logs --follow\n\n# rollback ל-deployment קודם\nvercel rollback\n\n# רשימת deployments\nvercel ls\n\n# מחיקת deployment ישן\nvercel remove <deployment-url>",
      },
    },
    {
      id: "features",
      ...
      content: [
        ...
        "Rollback מיידי — אם דחפתם באג ל-production, לחיצה אחת בדשבורד חוזרת ל-deploy הקודם. תוך שניות",
        ...
        "Image Optimization — `next/image` עובד אוטומטית. תמונות נטענות בפורמט WebP/AVIF, בגודל אופטימלי, באופן lazy",
        "Analytics — מציג top pages, top countries, devices ו-Core Web Vitals אמיתיים. ה-tier החינמי כולל מספר מוגבל של events בחודש",
      ],
      tips: [
        "הוסיפו ל-PR template את הקישור ל-preview deployment — למרות ש-Vercel מוסיפים קומנט אוטומטי, זה עדיין שווה",
        "Vercel Analytics נחמד, אבל חלקי — לדוחות עמוקים יותר, כדאי להוסיף גם Plausible/Umami",
        "Cron jobs ב-Vercel החינמי = פעם ביום בלבד. ל-cron תכוף יותר, צריך Pro",
      ],
      codeExample: {
    ...
    },
    {
      id: "edge",
      icon: Zap,
      title: "Edge Functions מול Serverless Functions",
      ...
      content: [
        ...
        "מתי serverless — חיבור ל-DB, library כבדה, פעולות שלוקחות יותר מ-30 שניות, או שצריכות יותר מסט הבסיס של Web APIs",
        "Streaming — Edge Functions תומכות ב-streaming responses. מצוין ל-AI streaming (OpenAI/Anthropic) — התשובה מתחילה להגיע מיידית, עוד לפני שהסתיימה",
      ],
      tips: [
        "אם האתר שלכם הוא בעיקר תוכן סטטי + מעט API, edge runtime יכול לתת latency נמוך באופן בלתי-נתפס (10-30ms עולמית)",
        "edge functions מוגבלות בזיכרון (128MB) ובזמן ריצה (30s). אם האפליקציה שלכם זקוקה ליותר — הישארו ב-serverless",
      ],
      codeExample: {
        label: "Next.js route עם edge runtime ל-AI streaming",
    ...
    },
    {
      id: "pricing",
      icon: Activity,
      title: "תמחור: מה ב-tier החינמי, ומתי לעבור ל-Pro",
      ...
      content: [
        ...
        "Image optimization — חינם ב-Hobby עד 1,000 source images. מעבר לזה, בחיוב",
        "Edge Middleware — מוגבל ל-1M invocations חינם. מעבר לזה, $0.65 לכל מיליון",
        "Vercel KV / Postgres / Blob — שירותים נפרדים בחיוב. בדרך כלל יקרים יותר מ-Supabase או Neon",
      ],
      tips: [
        "ודאו שיש Spend Limit ב-account settings. אצלי הוא קבוע ל-$5 — אם משהו פוצץ, אני מפסיד $5 מקסימום, לא $500",
        "Bandwidth = הסעיף הגדול. אצלי 5+ פרויקטים ב-Hobby עדיין מתחת ל-50% מה-100GB. אבל אתר ויראלי יכול לפוצץ את הכול ביום",
    ...
    },
    {
      ...
      content: [
        ...
        "Railway — אם אתם רוצים ענן עם backend מלא (Postgres, Redis), Railway הוא יותר 'Heroku מודרני'. עולה $5/חודש בסיס",
        "Render — דומה ל-Railway. עוד אלטרנטיבת PaaS ל-full-stack",
        "Fly.io — VMs קטנות באזורים. יותר שליטה, אבל יותר עבודה. טוב ל-backends",
  ...
  resources: [
    ...
    },
    {
      ...
      description: "מאות תבניות מוכנות ל-deploy בלחיצה",
      href: "https://vercel.com/templates",
      icon: ExternalLink,
    },
    {
      title: "Cloudflare Pages",
      description: "האלטרנטיבה הבולטת ביותר — bandwidth ללא הגבלה בחינם",
      href: "https://pages.cloudflare.com/",
      icon: ExternalLink,
    },
    {
      title: "Vercel CLI",
      description: "כלי command line ל-deploy וניהול",
  ...
  ctaTitle: "צריכים עזרה להעביר אתר ל-Vercel?",
  ctaSub:
    "מ-VPS עם nginx ל-Vercel = שעת עבודה. אני יכול להעביר את האתר שלכם, להגדיר custom domain ו-SSL.",
  primaryCta: {
