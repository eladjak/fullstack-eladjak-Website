...
export const postgresGuide: AgentGuideData = {
  ...
  stats: [
    { label: "גרסה", value: "16+" },
    { label: "שנות פיתוח רציף", value: "30+" },
  ...
  sections: [
    ...
    },
    {
      ...
      difficulty: "beginner",
      beginner:
        "תחשבו על זה כמו לבחור איפה לגור. אפשר לבנות בית לבד מאפס (apt-get + הגדרות ידניות), שליטה מלאה, אבל גם כל התחזוקה עליכם. אפשר לשכור דירה מרוהטת (Docker), נכנסים תוך דקה והכול כבר מחובר. ואפשר לקחת מלון עם שירות חדרים (Supabase, Neon), מישהו אחר מנקה, מגבה ומעדכן בשבילכם, ואתם רק משתמשים. למתחילים אני (אלעד) ממליץ על Docker בפיתוח ועל Supabase כשרוצים להעלות מהר לאוויר בלי לתחזק שרת.",
      content: [
    ...
    },
    {
      ...
      beginner:
        "סכמה זו בעצם התכנית האדריכלית של הבניין לפני שיוצקים בטון. אפשר לזרוק קירות איפה שבא לכם ולגלות אחר כך שאין מקום למדרגות, או לתכנן נכון מראש ולחסוך הריסות. הכלל שאני (אלעד) הולך לפיו: לא לנרמל יותר מדי בהתחלה (אל תפצלו כל דבר לעשר טבלאות לפני שצריך), אבל כן להשתמש בסוגי-נתונים נכונים מהיום הראשון, תאריך נשמר כתאריך, כסף נשמר כמספר עשרוני מדויק, לא כטקסט. לתקן את זה אחר כך זה כאב ראש; לעשות נכון מההתחלה זה חינם.",
      content: [
    ...
    },
    {
      ...
      difficulty: "intermediate",
      beginner:
        "הרחבות זה כמו אפליקציות שמתקינים בטלפון. הטלפון בסיסי יודע להתקשר ולשלוח הודעות, אבל ברגע שמתקינים אפליקציה הוא פתאום יודע גם לנווט, לערוך תמונות ולנהל כסף. ב-Postgres זה אותו רעיון: פקודה אחת (`CREATE EXTENSION`) ופתאום אותו בסיס נתונים יודע גם לחפש לפי משמעות (pgvector), גם להחזיק נתוני זמן (TimescaleDB) וגם למצוא לקוחות לפי מיקום על מפה (PostGIS). אצלי (אלעד) זה אומר שאני לא צריך חמישה שירותים נפרדים, הכול יושב במקום אחד.",
      content: [
    ...
    },
    {
      ...
      difficulty: "advanced",
      beginner:
        "דמיינו מסעדה. בפיתוח יש לכם שולחן אחד ולקוח אחד, קל. בייצור פתאום מאה אנשים רוצים לאכול בו-זמנית, ואם תיתנו לכל אחד מלצר צמוד תיגמר לכם המשכורת. הפתרון הוא מלצר אחד שמשרת כמה שולחנות בחוכמה, וזה בדיוק מה ש-PgBouncer עושה: מחזיק קומץ חיבורים אמיתיים ומחלק אותם בין אלפי משתמשים. כל שאר הסעיף כאן (אינדקסים חכמים, תצורת זיכרון, אבטחה) הוא הדברים שאף אחד לא חושב עליהם בפיתוח ושכולם לומדים בייצור, לרוב בדרך הקשה. שווה לקרוא לפני, לא אחרי.",
      content: [
      ...
      tips: [
        "אצלי Postgres רץ ב-docker עם חומת אש (iptables) שחוסמת את פורט 5432 לעולם החיצוני. גישה רק דרך SSH tunnel מהמחשב שלי, או דרך הרשת הפנימית של docker",
    ...
    },
    {
      ...
      difficulty: "advanced",
      beginner:
        "גיבוי זה כמו ביטוח, משעמם לשלם עליו, עד היום שבו הבית נשרף. ההבדל הוא שכאן יש מלכודת אכזרית: גיבוי שלא ניסיתם לשחזר ממנו הוא בעצם לא קיים, כי לרוב מגלים שהוא פגום בדיוק ברגע שצריך אותו. לכן הכלל אצלי (אלעד) הוא לא רק 'לגבות' אלא 'לבדוק פעם בחודש שהשחזור באמת עובד'. בסעיף הזה תכירו את שתי השיטות, אחת פשוטה ויומיומית (pg_dump) ואחת מתקדמת שמאפשרת לחזור בזמן לדקה מסוימת בעבר (PITR), ומתי כל אחת מתאימה.",
      content: [
  ...
  authorBio:
    "אני עובד עם Postgres מ-2014, ובכל פרויקט רציני שהקמתי הוא נמצא במרכז: מ-CRM ללקוחות, דרך מערכות אוטומציה ועד אתרים גדולים. אצלי היום הוא מגבה אפליקציות ספציפיות על השרת, כמו n8n ומערכת האנליטיקה, בעוד זיכרון הסוכנים חי ב-Qdrant ובקבצים פשוטים. המדריך הזה הוא תמצית של מה שלמדתי בדרך הקשה.",
};
