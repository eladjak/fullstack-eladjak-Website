...
export const cloudflareTunnelGuide: AgentGuideData = {
  ...
  agentNameHe: "Cloudflare Tunnel — חושפים שירות בלי לפתוח פורט",
  category: "infra",
  ...
  paradigmShifts: [
    {
      before: "פותחים פורט 443, מקווים שאין חולשה",
  ...
  sections: [
    {
      ...
      subtitle: "מי מתחיל את הקשר — ולמה זה משנה הכול",
      description:
      ...
      difficulty: "beginner",
      beginner:
        "תחשבו על זה ככה: בית עם דלת פתוחה (פורט 443) דורש שומר, חוקים ומנעולים. עם Tunnel, אין דלת בכלל — אבל יש לכם נציג חיצוני (Cloudflare) שיוצא מהבית דרך פתח אוורור, מקבל אורחים, ומעביר את ההודעות שלהם פנימה דרך תקשורת פנימית. גנבים יכולים לעמוד בחוץ כמה שירצו — אין דלת לפרוץ.",
      content: [
        "cloudflared — הדמון שרץ על השרת שלכם. בינארי קטן (~50MB) שכל מה שהוא עושה זה להחזיק חיבור פתוח ל-Cloudflare ולהעביר תעבורה. נדרש כשירות systemd בלבד",
        ...
        "DNS CNAME אוטומטי — Cloudflare יודעים ליצור את ה-DNS record של המנהרה אוטומטית. אתם רק מציינים בפקודה אחת ש-`hub.example.com` שייך למנהרה X",
        "אין צורך ב-IP ציבורי — השרת יכול להיות ב-NAT של ספק האינטרנט בבית, או על Raspberry Pi עם IP דינמי. המנהרה עובדת כל עוד יש חיבור החוצה",
    ...
    },
    {
      ...
      difficulty: "beginner",
      beginner:
        "מה שמפתיע אנשים זה כמה זה מהיר — מרעיון לדומיין חי באוויר תוך כחמש דקות. הצעדים נראים הרבה כי כתבתי אותם בפירוט, אבל בפועל זה כמו להרכיב רהיט מ-IKEA: כל שלב הוא פעולה אחת קטנה וברורה. מתקינים את התוכנה, מתחברים לחשבון, יוצרים מנהרה, מצביעים עליה דומיין — וזהו, יש לכם אתר חי בלי לפתוח אף פתח בשרת. אצלכם זה אומר שגם שרת שיושב בבית מאחורי הראוטר, בלי כתובת ציבורית, יכול להגיש אתר לכל העולם.",
      content: [
      ...
      tips: [
        "אם הדומיין שלכם אצל ספק אחר (Namecheap, GoDaddy, isoc.org.il לדומיינים ישראליים), קודם העבירו את ה-nameservers ל-Cloudflare. הם נותנים לכם 2 NS שאתם מציבים אצל הרשם",
        "דומיינים ישראליים `.co.il` עובדים מצוין עם Cloudflare. נדרש להעביר nameservers דרך isoc.org.il — תהליך של 24 שעות אבל חופשי",
        "Cloudflare Zero Trust דורש הפעלה חד-פעמית של 'Cloudflare Tunnels' בדשבורד תחת 'Zero Trust'. ה-tier החינמי כולל 50 משתמשים",
      ],
      codeExample: {
    ...
    },
    {
      ...
      subtitle: "אותה מנהרה — מאות שירותים, אם רוצים",
      description:
      ...
      beginner:
        "קובץ ה-config הוא בעצם רשימת הניתוב של המנהרה — לוח מודעות שאומר 'מי שמחפש את הכתובת הזאת, שלח אותו לחדר ההוא'. בזכותו, מנהרה אחת יכולה לשרת עשרות דומיינים שונים, כל אחד מוביל לשירות אחר על השרת. אצלי (אלעד) מנהרה יחידה משרתת יותר מ-10 דומיינים, כל אחד מגיע לקונטיינר אחר — והכל מוגדר בקובץ טקסט אחד קצר. יש כלל אחד ששווה לזכור: תמיד לסיים בשורה 'אם שום דבר לא התאים — תחזיר 404', כדי שלא יישאר פתח לא מוגדר.",
      content: [
        "ingress — מערך של rules. כל rule נבדק לפי הסדר; הראשון שמתאים — מבצע. תמיד שמים rule של catch-all בסוף עם `service: http_status:404`",
        ...
        "hello-world — שירות מובנה לבדיקות. `service: hello_world` נותן דף Cloudflare שמראה שהמנהרה עובדת",
        "עדכון חם — כדי לעדכן config בלי downtime: `systemctl reload cloudflared`. הוא קורא את הקובץ מחדש בלי להפיל חיבורים",
      ],
      tips: [
        "תמיד הוסיפו `path: \"^/(metrics|admin)\"` עם `service: http_status:403` כדי לחסום endpoints רגישים — אפילו אם השירות עצמו לא מגן עליהם",
        ...
        "ב-VS Code יש תוסף 'Cloudflare Tunnel' שמראה את המנהרות הפעילות ומאפשר לערוך config מתוך ה-IDE",
      ],
      codeExample: {
    ...
    },
    {
      ...
      difficulty: "intermediate",
      beginner:
        "כאן נכנס פיצ'ר ששווה זהב: Cloudflare יכולה לדרוש מהמבקר להוכיח מי הוא — עוד לפני שהבקשה בכלל מגיעה לאתר שלכם. דמיינו מאבטח בכניסה לבניין שבודק תעודה לפני שנותן לכם להיכנס למעלית. אצלי (אלעד) הדשבורד הפרטי של רשת הסוכנים מוגן ככה: רק אימייל אחד (שלי) מורשה להיכנס, וכל השאר נחסמים בדלת. היופי הוא שלא צריך לבנות מערכת התחברות באפליקציה — Cloudflare עושה את כל העבודה, חינם, עד 50 משתמשים. מושלם לכלים פרטיים שרוצים להגיע אליהם מכל מקום אבל רק אתם.",
      content: [
        "Application — בדשבורד Zero Trust, יוצרים 'application' שמייצג את הדומיין/path. למשל `dashboard.example.com`",
        "Policies — חוקים על מי מורשה. 'Allow if email ends with @company.com', 'Allow if user is in GitHub org X', 'Require MFA'",
        "Identity providers — Google, GitHub, GitLab, Okta, Azure AD, SAML — כל הגדולים נתמכים בחינם",
        "JWT validation — אחרי שמשתמש עובר אימות, Cloudflare מצרפים JWT לבקשה. השירות שלכם יכול לבדוק אותו אם רוצה לדעת מי המשתמש",
        "Service tokens — לאוטומציה (CI/CD, סקריפטים). מקבלים client_id + client_secret במקום אימות אנושי",
        "Browser-based SSH — Cloudflare מאפשרים להתחבר ל-SSH דרך הדפדפן, עם אימות SSO. היתרון: אין צורך להחזיק SSH key פיזי",
        "Audit log — Cloudflare מתעדים כל login. בדשבורד שלהם אפשר לראות מי נכנס, מאיפה ומתי",
      ],
      tips: [
        ...
        "Access חוסם _לפני_ שהבקשה מגיעה למנהרה. זו שכבת אבטחה נוספת — אם יש חולשה באפליקציה, התוקף בכלל לא מגיע אליה",
    ...
    },
    {
      ...
      difficulty: "intermediate",
      beginner:
        "עד כאן דיברנו על איך זה עובד; כאן רואים למה זה שימושי בחיים האמיתיים. הרעיון המשותף לכל הדוגמאות פשוט: בכל פעם שיש לכם משהו 'פנימי' שאתם רוצים להראות לעולם בלי לפתוח דלת מסוכנת. לחשוף שרת מדיה מהבית בלי לגעת בראוטר; להראות ללקוח אתר שרץ אצלכם על המחשב, בלחיצה אחת; לקבל התראות ממערכת חיצונית תוך כדי פיתוח. אצלי (אלעד) זה אפילו מה שמחבר את כל רשת הסוכנים לאינטרנט. הכלי אחד — השימושים רבים.",
      content: [
        ...
        "מספר אזורים — אצלי (אלעד) hub.eladjak.com מוגש מ-Cloudflare datacenter בתל אביב למשתמשים בישראל, ומפרנקפורט למשתמשים באירופה. אוטומטית, בחינם",
      ...
      tips: [
        ...
        "מנהרות הן idempotent — אם קובץ ה-config מתחלף, השדרוג חלק. אפשר לשנות routing בייצור בלי downtime",
      ],
    },
    {
      id: "vs-alternatives",
      ...
      difficulty: "intermediate",
      beginner:
        "Cloudflare Tunnel הוא לא היחיד שעושה את הקסם הזה, וכדאי להכיר את החברים שלו כי לכל אחד תפקיד טוב יותר במצב אחר. ngrok נהדר לבדיקות מהירות בזמן פיתוח; Tailscale מצוין לחבר בין כמה מחשבים פרטיים שלכם (לפטופ, שרת בבית) בלי לחשוף לעולם בכלל. הכלל שלי (אלעד) פשוט: לחשיפה ציבורית של אתר על שרת — Cloudflare Tunnel. למשרד עם כמה מכונות שצריכות לדבר ביניהן — Tailscale. ואחת אזהרה חשובה: אל תסתמכו על שירות חינמי אחד לכל החיים — תמיד שיהיה לכם תוכנית מילוט אם הוא נופל.",
      content: [
        "ngrok — האלטרנטיבה הוותיקה. החינמי מוגבל ל-URL שמשתנה ולחיבור אחד. ל-dev ול-webhooks קצרים מצוין; לייצור — צריך תשלום",
        "Tailscale — VPN mesh. מצוין לחיבור בין מכונות פרטיות (lab, מחשב בית, VPS), אבל לא חושף לעולם הרחב — דורש שכל הצדדים יתחברו ל-tailnet",
        "FRP / WireGuard עצמי — open source, free. דורש שרת cloud עם IP ציבורי שמשמש כ-relay. יותר עבודה, אבל אפס תלות בספק",
        "Inlets — אלטרנטיבה אירופית עם מודל פתוח יותר. דורשת שרת relay משלכם",
        ...
        "השוואה מהירה: ל-dev local → ngrok או cloudflared חינמי. לייצור על VPS → Cloudflare Tunnel. למשרד עם כמה מכונות → Tailscale. להגדרה עצמאית מלאה → WireGuard",
      ],
      tips: [
        ...
        "אל תסתמכו על שירות זר אחד לכול. חינם זה נחמד, אבל גיבוי = יכולת לעבור ל-nginx + פורט פתוח אם Cloudflare נופל",
        "Cloudflare Pro ($20/חודש) פותח פיצ'רים נוספים: WAF rules, image optimization, mobile optimization. שווה אם האתר הוא מקור הכנסה",
      ],
    },
  ],
  resources: [
    ...
    },
    {
      ...
      description: "אחת ההצעות החינמיות הנדיבות באינטרנט — פירוט יכולות",
