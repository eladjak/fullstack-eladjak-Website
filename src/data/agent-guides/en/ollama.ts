...
export const ollamaGuideEn: AgentGuideData = {
  ...
  sections: [
    ...
    },
    {
      ...
      description:
        "Integration is the point where Ollama goes from being a nice local tool to becoming a beating part of a larger system. My agent network's code carries exactly such a fallback layer (a backup plan) — it knows how to switch to a local model when the cloud is unavailable. Full disclosure: that layer is not active on the server right now (Ollama is only installed on my workstation), but the examples here show exactly how to wire it up. Thanks to the OpenAI-compatible endpoint, every model in the network can swap from Claude or Gemini to Ollama with just a URL change — especially useful for classification tasks inside [Adopter](/en/guide/adopter) and for triaging intakes in [Box](/en/guide/box).",
      color: "from-rose-600 to-pink-500",
      difficulty: "advanced",
      beginner:
        "Here Ollama stops being a nice local toy and becomes a working part of a real system. The role I designed for it (Elad) is twofold: a safety net (if the cloud model is overloaded or costs money, the task gracefully falls back to the free local one), and a quiet background worker for simple tasks not worth paying the cloud for — like sorting posts or tagging data. Today Ollama runs on my workstation, and the server-side layer is written and waiting to be switched on. Since its API is identical to ChatGPT's, any agent in the network can switch to it with just a URL change. The idea: keep the expensive models for tasks that genuinely need them, and let everything else run for free.",
      content: [
        ...
        "Fallback in [Kami](/en/guide/kami): the code carries a layer where, if Claude returns an error (429 is a quota exceeded, 500 is a server fault), it catches the exception and switches to a local model — the conversation continues without the user noticing. In my setup it is written and ready, but inactive on the server as long as Ollama isn't installed there",
        "Bulk classification tasks — like [Adopter](/en/guide/adopter)'s — are natural candidates for a local model, instead of burning cloud quota. In my setup the classification currently runs on Gemini's free tier, but this is exactly the kind of task Ollama can take over",
  ...
  authorBio:
    "Ollama runs on my workstation, and the fallback layer to it exists in my agent network's code — written and ready to switch on the day I need it. This guide lays out the practical split: which models are worth running local, when to go hybrid, and how to integrate with LangChain/CrewAI without breaking existing workflows.",
};
