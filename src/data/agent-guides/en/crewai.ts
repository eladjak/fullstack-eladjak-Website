...
export const crewaiGuideEn: AgentGuideData = {
  ...
  sections: [
    ...
    },
    {
      ...
      beginner:
        "The easiest way to grasp CrewAI is to think of it as managing a team of workers — only the workers are AI. You have an 'agent' (a worker with its own role and personality, say a 'researcher' or a 'writer'), a 'task' (a clear work order), and a 'crew' (how they all work together). And it's like Lego: once you understand those four pieces, you can assemble any process you can imagine from them. The beauty is that it's surprisingly simple — for me (Elad) a whole crew of several cooperating 'workers' is written in just a few dozen lines.",
      content: [
    ...
    },
    {
      ...
      difficulty: "advanced",
      beginner:
        "Until now the team worked in a chain — one finishes and hands off to the next, like an assembly line. Here you add a 'manager': an agent that doesn't do the work itself, but decides in real time who does what, when to split a task, and when to stop. Exactly like an orchestra conductor who doesn't play but determines who comes in when. It's a more flexible and powerful mode, but also more expensive and less predictable — which is why for me (Elad) only one crew in ten works this way, and all the rest use the simple chain. The rule: start simple, and move to a 'manager' only when you genuinely need the flexibility.",
      content: [
