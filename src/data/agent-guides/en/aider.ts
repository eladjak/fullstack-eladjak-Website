...
export const aiderGuideEn: AgentGuideData = {
  ...
  sections: [
    ...
    },
    {
      ...
      description:
        "The most important step for serious Aider use is installing it in isolation — meaning a separate Python environment with API keys that are separate from [Claude Code](/en/claude-code). An API key is a secret code that identifies your account with the AI provider (Anthropic, OpenAI, Google, etc.) and is what you get billed on. On my setup Aider lives in a dedicated folder `/opt/elad-network/agents/aider-assist` with a Gemini key that is completely separate from my [Claude Max](/en/claude-code) account — so working with Aider never dips into the primary tool's quota.",
      color: "from-slate-600 to-zinc-500",
      difficulty: "intermediate",
      beginner:
        "Why go to all this trouble to isolate things? Think of it like two bank accounts: one for work, one for savings. If they share a credit card, every purchase eats into the savings too. Same idea here — if Aider uses the same API keys as [Claude Code](/en/claude-code), every line of code it writes comes out of the same monthly quota. Full isolation = peace of mind. My Aider runs on a separate free Gemini key, while [Claude Code](/en/claude-code) keeps humming along on the primary subscription. (An excellent alternative for the same principle: OpenRouter — a gateway offering free models like Qwen 2.5 Coder:free and DeepSeek V3:free.)",
      content: [
    ...
    },
    {
      ...
      difficulty: "intermediate",
      beginner:
        "Aider is essentially a 'driver', and the AI model is the 'engine' you choose to drop into it — and there are plenty of engines, from the premium and powerful down to the free one that does 80% of the work. The important idea to remember: you don't need the most expensive engine for every trip. Just as you wouldn't take a private jet to pop to the corner shop, there's no point burning the pricey model on fixing a typo. For me (Elad) the strong model is reserved for complex tasks, and for small everyday work I run a cheap or free one — which is what slashes the bill without hurting quality.",
      content: [
    ...
    },
    {
      ...
      difficulty: "intermediate",
      beginner:
        "Don't let the list of commands scare you — in practice, daily work with Aider is a simple conversation, like with an assistant. You point at a file ('let's work on this'), ask in plain language ('add a check that the email is valid'), and it edits, shows you exactly what changed, and saves. The short slash commands are just shortcuts — knowing seven or eight of them is enough to keep the work flowing. The most useful of all is /undo: an 'undo' button that rolls everything back if the model missed. For me (Elad) that's what lets me work without fear.",
      content: [
    ...
    },
    {
      ...
      difficulty: "advanced",
      beginner:
        "Until now Aider was a tool you talk to by hand; here it becomes a worker that runs on its own. The big capability is that you can give it a single command with no conversation at all — 'fix every style error in this file' — and it does it and exits. That opens the door to automation: for example, every time someone proposes a code change, Aider goes over it automatically and cleans it up overnight, without anyone sitting at the screen. For me (Elad) it's essentially the network's 'code surgeon' — a component that takes a precise task from another agent, performs it carefully with safety nets, and returns the result for review. This section is more technical, but the idea is simple: the tool that worked beside you can also work for you.",
      content: [
        ...
        "Safety gates: branch-only never main · dry-run→per-task approval · file allowlist (no secrets/config/deploy) · max-diff-size + auto-rollback · visible logs. And its write gate is closed by default, on purpose — opened manually only when there is a real task. Its jobs: mechanical refactors, dependency bumps, lint/type fixes, test generation, and a 'second-implementation' alternate diff for comparison.",
    ...
    },
    {
      ...
      difficulty: "beginner",
      beginner:
        "The most common question is 'Aider or Claude Code?', and the honest answer is: it's not either-or, they complement each other. Like the difference between a fully-loaded off-road vehicle and a nimble motorcycle — each has a situation where it wins. My simple rule (Elad): if you have a [Claude Max](/en/claude-code) subscription, that's the primary tool for most of the work, thanks to all the smart capabilities in it. If you don't, or you need to work for free or on sensitive code that mustn't leave the machine — Aider takes the stage and still delivers high-quality results. There's no single winner here; there's the right tool for each situation.",
      content: [
