...
export const aiderGuideEn: AgentGuideData = {
  ...
  heroDescription:
    "Aider is an AI pair-programming CLI written in Python. It supports 100+ models (Anthropic, OpenAI, Gemini, OpenRouter, [Ollama](/en/guide/ollama)), edits files directly on disk, creates automatic git commits, and includes a smart repo-map that understands your project. On my machine Aider is installed in an isolated environment with separate credentials (not [Claude Max](/en/claude-code)) and free models (qwen3-coder:free via OpenRouter) — a backup for when Claude Max hits its quota or when I want an extra-private session. For you, Aider can be the primary tool: if you don't have Claude Pro/Max, or if you're a privacy-focused developer who needs local-only inference (with [Ollama](/en/guide/ollama)), Aider delivers about 80% of [Claude Code](/en/claude-code)'s capability for absolutely zero cost.",
  ...
  stats: [
    { label: "Supported models", value: "100+" },
  ...
  paradigmShifts: [
    {
      ...
      after: "Aider + OpenRouter qwen3:free = $0, full stop",
      icon: DollarSign,
    },
    {
    ...
    },
    {
      ...
      after: "Aider + Ollama qwen3-coder = fully air-gapped",
      icon: Terminal,
    },
  ],
  whoIsThisFor: [
    {
      ...
      description:
        "Want free AI pair programming — OpenRouter with qwen3-coder:free delivers most of the value.",
      icon: DollarSign,
  ...
  sections: [
    {
      ...
      content: [
        ...
        "Supports 100+ models via a library called LiteLLM (a kind of \"universal translator\" between different AI providers). You can pick [Claude](/en/claude-code) from Anthropic, GPT-4 from OpenAI, Gemini from Google, local models via [Ollama](/en/guide/ollama), or free models via OpenRouter (a payment gateway that aggregates dozens of providers).",
    ...
    },
    {
      ...
      beginner:
        "Why go to all this trouble to isolate things? Think of it like two bank accounts: one for work, one for savings. If they share a credit card, every purchase eats into the savings too. Same idea here — if Aider uses the same API keys as [Claude Code](/en/claude-code), every line of code it writes comes out of the same monthly quota. Full isolation = peace of mind. Aider runs on a free OpenRouter account (a payment gateway that offers free models like Alibaba's qwen3-coder:free) while [Claude Code](/en/claude-code) keeps humming along on the primary subscription.",
      content: [
        ...
        "Fully separate account: go to openrouter.ai, sign up for free, generate an API key, and use it with qwen3-coder:free. This model is open-source from Alibaba, impressively capable, and costs nothing — perfect as a backup.",
        "A configuration file `~/.aider.conf.yml` tells Aider which default model to pick and where to find it. For example: `model: openrouter/qwen3-coder:free` + `openai-api-base: https://openrouter.ai/api/v1`. That way every time you launch Aider it already knows where to connect.",
        ...
        "Want full privacy? Set `model: ollama/qwen3-coder` + `api_base: http://localhost:11434`. [Ollama](/en/guide/ollama) is a tool that runs AI models directly on your own machine, without sending a single line of code to an external server. Perfect for sensitive domains like medical, legal, or confidential company code.",
      ],
      tips: [
    ...
    },
    {
      ...
      description:
        "Aider can connect to more than 100 different AI models through a library called LiteLLM (think of it as a \"universal adapter\" that translates requests for each provider separately). In practice, most developers stick with 5-7 models that cover 95% of their needs. In this section we'll walk through them by category — from the priciest and strongest down to the free alternatives that still do 80% of the work. Choosing well saves a lot of money: you don't want to burn the top model on fixing a typo.",
      color: "from-indigo-600 to-blue-500",
      difficulty: "intermediate",
      content: [
        "[Claude Sonnet 4.6](/en/claude-code) from Anthropic — currently the best model for complex code editing, especially in large projects with many interdependent files. Cost: about $3 per million input tokens (one million tokens ≈ 750,000 words). This is a model that handles long context and does refactoring (reorganizing code) smoothly. It's my primary choice for [Claude Code](/en/claude-code).",
        "GPT-4.1 and o3-mini from OpenAI — excellent for problems that demand deep reasoning and for changes that touch many files at once. GPT-4.1 is the full-size version; o3-mini is the snappier, cheaper sibling. Useful when you want a second opinion on something Claude is struggling with.",
        "Gemini 2.5 Pro from Google — comparable quality to GPT-4 at a significantly lower price. Especially brilliant for mass fixes (batch fixes) — for example, \"go through 50 files and replace the old function with the new one in each.\" Thanks to its long context (a million tokens) it can hold entire projects in RAM.",
        "qwen3-coder:free from Alibaba, via OpenRouter — the best free code model available today. Slower than paid models and limited in context (the amount of text it can remember at once), but absolutely free. On my setup this is the backup when [Claude Max](/en/claude-code) hits its quota. For you — if you don't have Claude Pro/Max, this can be your primary tool.",
        "deepseek-coder-v3 from DeepSeek — particularly strong on Python. A little less sharp on TypeScript, so not ideal for every project. Free on OpenRouter's basic tier.",
        "[Ollama](/en/guide/ollama) with local qwen3-coder or deepseek-coder — the most private option available. Code never leaves your machine, there's no log at any cloud provider, and it's perfect for sensitive industries (medical, legal, defense). The one requirement: a computer with a strong GPU (graphics processing unit). Without a good GPU it's very slow.",
        "The `--weak-model` flag — a clever cost-saver. Aider uses a small, simple model for writing commit titles (the short message describing what changed). There's no reason to burn expensive Sonnet on \"fix: typo in login\" — point weak-model at a free model and you'll save 30-40% on total cost.",
      ],
      tips: [
        "The `--edit-format diff` flag fits most projects — it sends the LLM only the change itself, not the whole file. For large, complex changes, `--edit-format udiff` is better (a more sophisticated diff format). Sonnet handles both better than any other model.",
        "Before launching a large request through Aider, run `aider --msg '/tokens'` — it shows you how many tokens are currently in context and the estimated cost of another request. About 30K tokens is a normal conversation, 100K is already expensive, and over 200K is a gamble on your wallet.",
        "For simple problems (typos, single-line fixes, variable renames) — use haiku from Anthropic or qwen3-coder:free. No sense burning Opus, Anthropic's most expensive model, on a five-minute bug.",
      ],
      codeExample: {
    ...
    },
    {
      ...
      tips: [
        ...
        "`/model claude-sonnet-4-6` swaps the model mid-session. Useful when the weaker model is failing and you need to \"upgrade\" to something more serious — or the other way around, when you finished the hard part and want to drop back to a cheap model for small tasks.",
        "`/voice` — if you set up OpenAI Whisper (a speech-to-text service), you can dictate requests out loud instead of typing. About 2x faster, especially in Hebrew. For me it saves a ton of time during long sessions in front of the screen.",
      ],
      codeExample: {
    ...
    },
    {
      ...
      content: [
        ...
        "Monthly cost: [Claude Code](/en/claude-code) requires a Max subscription ($100-200/month) or a pay-as-you-go API key. Aider can be completely free with qwen3-coder:free, or low-cost with OpenRouter pay-as-you-go.",
      ...
      tips: [
        ...
        "If you don't have [Claude Max](/en/claude-code) — start with Aider + OpenRouter qwen3-coder:free. You'll get 80% of the capability for free. When you hit a request Aider can't handle, consider moving to [Claude Code](/en/claude-code).",
      ],
    },
  ],
  resources: [
    ...
    {
      title: "OpenRouter",
      description: "API gateway for 100+ models including qwen3-coder:free",
  ...
  authorBio:
    "Aider complements Claude Code for isolated tasks: lint fixes, small refactors, and CI automations. The main advantage — support for 100+ models including free (OpenRouter) and local (Ollama) ones, so you can build a hybrid that saves on cost without giving up quality. This guide walks through the isolated setup, picking the right model per task, and how to fit Aider into a daily workflow or an automated pipeline.",
};
