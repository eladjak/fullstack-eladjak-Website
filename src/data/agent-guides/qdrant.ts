...
export const qdrantGuide: AgentGuideData = {
  ...
  heroDescription: "Qdrant זה סוג של בסיס נתונים חדש יחסית שמכונה 'בסיס נתונים וקטורי' — כזה שיודע לזכור לפי משמעות ולא לפי מילים. בניגוד לבסיסי נתונים מסורתיים (כמו Postgres או MySQL) שמחפשים התאמה מדויקת של טקסט — 'תמצא לי את כל הלקוחות ששמם דוד' — Qdrant יכול לחפש רעיונות: 'תמצא לי שיחות דומות לזו, גם אם השתמשו במילים אחרות'. זו היכולת שמאפשרת למערכות AI מודרניות (ChatGPT, [Claude](/claude-code), Perplexity) לזכור את השיחות הקודמות שלכם ולהחזיר תשובה רלוונטית, גם כשהניסוח שונה לחלוטין. Qdrant בקוד פתוח לחלוטין, נכתב בשפת Rust (אחת השפות המהירות בעולם התוכנה, אותה שפה שבה כתובים דפדפנים מודרניים), ורץ כשירות רקע על השרת — בדיוק כמו מסד נתונים רגיל, רק שבמקום טבלאות הוא מאחסן 'טביעות אצבע מתמטיות' של משפטים (מה שנקרא embeddings). אצלי (אלעד) Qdrant הוא הלב של כל רשת הסוכנים: הוא מחזיק את הזיכרון של [Kami](/guide/kami) בוואטסאפ, את היומן של [Box](/guide/box), ואת המאגר של [Adopter](/guide/adopter). אצלכם Qdrant יכול להיות הבסיס של צ'אטבוט שזוכר לקוחות, של חיפוש חכם באתר, של מערכת המלצות, או של כל מקום שצריך 'לזכור לפי רעיון ולא לפי מילה'.",
  ...
  sections: [
    ...
    },
    {
      ...
      content: [
        ...
        "OpenAI text-embedding-3-small — עולה בערך $0.02 למיליון מילים, 1536 מימדים (מתאים אם אתם כבר משלמים ל-OpenAI)",
        "Voyage voyage-3 — נחשב הכי איכותי לאנגלית בעולמות ה-RAG, אבל יקר יותר ולא חינמי",
        "Cohere embed-multilingual-v3 — מודל מעולה שתומך בלמעלה מ-100 שפות, כולל עברית ברמה טובה",
