  ...
  sections: [
    {
      ...
      description:
        "Aider הוא כלי פתוח מבית החוקר Paul Gauthier — מעין 'עוזר תכנות בזוג' שרץ מתוך חלון הטרמינל (הממשק השחור-לבן עם הפקודות). הוא כתוב בשפת Python ומתחבר ל-LLM (מודל שפה גדול — כמו [Claude](/claude-code), GPT של OpenAI או Gemini של Google) כדי להבין מה אתם רוצים ולממש את זה ישירות בקבצי הקוד שלכם. בניגוד לצ'אט רגיל שבו אתם מקבלים טקסט ומעתיקים ידנית — Aider עורך את הקבצים בעצמו, מריץ בדיקות אם תבקשו, ואפילו מבצע commit (שמירת נקודת ציון בהיסטוריית ה-git של הפרויקט) עם הודעה שהוא כותב לבד.",
      color: "from-orange-600 to-red-500",
      difficulty: "beginner",
      beginner:
        "חשבו על זה כאילו יש לכם שותף לתכנות שיושב לידכם, אבל במקום לדבר איתו אתם מקלידים לו בקשה בעברית או באנגלית — והוא פותח את הקובץ בעצמו, מתקן את מה שצריך ומסמן בהיסטוריה של הפרויקט איזה שינוי הוא עשה ולמה. פותחים טרמינל, נכנסים לתיקיית הפרויקט, מריצים `aider` ואומרים משהו כמו 'תקן את הבאג בקובץ login.ts' — תוך שניות הקובץ השתנה. רוצים להתחרט? פקודה אחת (`/undo`) מחזירה את הכל אחורה. זה בדיוק כמו לעבוד עם [Claude Code](/claude-code), רק ש-Aider הוא האלטרנטיבה החינמית והפשוטה יותר.",
      content: [
        "Aider הוא כלי CLI בשפת Python (CLI = Command Line Interface, ממשק שורת פקודה — הכלים שרצים בחלון הטרמינל השחור). ההתקנה היא פקודה אחת: `pipx install aider-chat`. למה pipx ולא סתם pip? כי pipx יוצר לכלי סביבה משלו שלא מתנגשת עם פרויקטים אחרים במחשב — בדיוק מה שרוצים לכלי שמותקן גלובלית.",
        ...
        "Auto-commit — כל שינוי ש-Aider עושה נשמר אוטומטית ב-git (מערכת ניהול הגרסאות של הקוד) עם הודעה ברורה: מה השתנה, איזו בקשה ביקשתם ובאיזה מודל השתמש. זה אומר שגם אם משהו ישתבש, תמיד יש לכם 'כפתור undo' מלא של כל צעד וצעד.",
        "Diff view (תצוגת הבדלים) — לפני כל שינוי Aider מראה לכם בבירור מה הוא עומד לעשות: מה יימחק באדום, מה ייכנס בירוק. אם לא אהבתם — `/undo` וזה חוזר אחורה. התחושה היא כמו לעבוד עם שותף שמבקש אישור לכל פעולה.",
      ...
      tips: [
        "אצלי (אלעד) Aider עובד הכי טוב על פרויקטים שמסודרים יפה: יש להם git תקין, typings ברורים ותיקיית tests. ככל שהפרויקט מאורגן יותר — כך ה-LLM מבין אותו מהר יותר ונותן תשובות מדויקות יותר.",
    ...
    },
    {
      ...
      description:
        "השלב החשוב ביותר בשימוש רציני ב-Aider הוא להתקין אותו מבודד — כלומר, בסביבת Python נפרדת ועם מפתחות API נפרדים מ-[Claude Code](/claude-code). מפתח API הוא קוד סודי שמזהה את החשבון שלכם אצל חברת ה-AI (Anthropic, OpenAI, Google וכו') ועליו מחייבים. אצלי (אלעד) Aider מותקן בתיקייה ייעודית `/opt/elad-network/agents/aider-assist` עם מפתח Gemini נפרד לגמרי מחשבון [Claude Max](/claude-code) הראשי — ככה עבודה ב-Aider לא נוגעת במכסה של הכלי המרכזי.",
      color: "from-slate-600 to-zinc-500",
      difficulty: "intermediate",
      beginner:
        "למה כל הטרחה הזו של בידוד? חשבו על זה כמו על שני חשבונות בנק: אחד לעבודה ואחד לחיסכון. אם שניהם על אותו כרטיס אשראי, כל קנייה שורפת גם מהחיסכון. אותו דבר פה — אם Aider משתמש באותם מפתחות API של [Claude Code](/claude-code), כל שורת קוד שהוא יכתוב תיגרע מאותה מכסה חודשית. בידוד מלא = שקט נפשי. אצלי Aider רץ על מפתח Gemini חינמי נפרד, בזמן ש-[Claude Code](/claude-code) ממשיך לעבוד לו בצד על המנוי הראשי. (אלטרנטיבה מצוינת לאותו עיקרון: OpenRouter — שער שמציע מודלים חינמיים כמו Qwen 2.5 Coder:free ו-DeepSeek V3:free.)",
      content: [
        "התקנה נקייה: `pipx install aider-chat` — הפקודה הזו יוצרת ל-Aider סביבת Python משלו בתיקייה `~/.local/pipx`, כך שלא יתנגש עם ספריות של פרויקטים אחרים. pipx הוא כלי שבנוי בדיוק להתקנה גלובלית מבודדת של כלי CLI בשפת Python.",
        "חשבון נפרד לחלוטין: נכנסים ל-openrouter.ai, נרשמים בחינם, מייצרים API key ומשתמשים בו עם מודל כמו `qwen/qwen-2.5-coder-32b-instruct:free` או `deepseek/deepseek-chat-v3:free`. המודלים האלה הם בקוד פתוח (Qwen של Alibaba Cloud, DeepSeek של DeepSeek), באיכות מרשימה ובעלות אפסית — מושלמים כ-backup. שימו לב שה-free tier של OpenRouter מוגבל ב-RPM (בקשות לדקה) ובמכסה יומית; מעבר לכך עוברים ל-tier בתשלום.",
        "קובץ הגדרות `~/.aider.conf.yml` שאומר ל-Aider איזה מודל לבחור כברירת מחדל ואיפה לחפש אותו. לדוגמה: `model: openrouter/qwen/qwen-2.5-coder-32b-instruct:free` + `openai-api-base: https://openrouter.ai/api/v1`. ככה בכל פעם שתפעילו את Aider הוא כבר יידע לאן לפנות.",
        "הערה קריטית: אל תגדירו את משתנה הסביבה `ANTHROPIC_API_KEY` באותו טרמינל שבו רץ Aider — אחרת Aider יבחר בו אוטומטית (זו ברירת המחדל שלו) וישרוף לכם את מכסת [Claude](/claude-code) בלי שתשימו לב. או שתריצו את Aider מתוך סקריפט עם `env -i` (שמריץ עם משתני סביבה נקיים), או שתשימו את המפתחות בקובץ `.aider.conf.yml` עצמו ולא ב-env.",
        "אם אתם דווקא רוצים להשתמש ב-Anthropic דרך Aider — ייצרו מפתח API נפרד בדשבורד של Anthropic, הגדירו לו תקרת חיוב (billing cap) של $10/חודש, וזה יהיה הבאפר הבטוח שלכם. אם המפתח הזה יישרף — [Claude Code](/claude-code) הראשי ממשיך לעבוד ללא הפרעה.",
        "רוצים פרטיות מלאה? הגדירו `model: ollama/qwen2.5-coder:32b` (או `qwen3-coder` אם התקנתם את Qwen 3 Coder) + `api_base: http://localhost:11434`. [Ollama](/guide/ollama) הוא כלי שמריץ מודלי AI ישר על המחשב שלכם, בלי לשלוח שום שורת קוד לשרת חיצוני. מושלם לתחומים רגישים כמו רפואה, משפט או קוד ארגוני סודי.",
      ],
      tips: [
        "אצלכם ב-Windows או ב-WSL (Windows Subsystem for Linux, סביבת לינוקס בתוך חלונות): הפקודה `pipx install aider-chat` בדרך כלל עובדת חלק. אם נתקלים בבעיות תלויות, גיבוי טוב הוא להתקין דרך conda — מנהל חבילות חלופי: התקינו `pipx install conda` קודם, ואז התקינו את Aider בתוכו.",
        ...
        "אל תשכחו להוסיף `aider-chat-logs/*` לקובץ `.gitignore` של הפרויקט. Aider שומר logs מפורטים של כל שיחה, ואלה לא צריכים להגיע ל-PR (Pull Request, בקשה למיזוג קוד ל-branch ראשי). זה גם חוסך מקום וגם מונע חשיפה של פרטים פנימיים.",
      ],
      codeExample: {
    ...
    },
    {
      ...
      difficulty: "intermediate",
      beginner:
        "Aider הוא בעצם 'נהג', ומודל ה-AI הוא ה'מנוע' שאתם בוחרים להכניס לו — ויש המון מנועים, מהיוקרתי והחזק ועד החינמי שעושה 80% מהעבודה. הרעיון החשוב לזכור: לא צריך את המנוע הכי יקר לכל נסיעה. כמו שלא ממריאים במטוס פרטי כדי לקפוץ למכולת, אין טעם לשרוף את המודל היקר על תיקון טעות כתיב. אצלי (אלעד) המודל החזק שמור למשימות מורכבות, ולעבודה היומיומית הקטנה אני מפעיל מודל זול או חינמי — וזה מה שמוריד את החשבון פלאים בלי לפגוע באיכות.",
      content: [
        "[Claude Sonnet 4.6](/claude-code) של Anthropic — המודל הטוב ביותר כיום לעריכת קוד מורכבת, במיוחד בפרויקטים גדולים עם הרבה קבצים שתלויים זה בזה. העלות: כ-$3 לכל מיליון טוקני קלט ו-$15 לכל מיליון טוקני פלט (מיליון טוקנים = בערך 750 אלף מילים). כאן מדובר במודל שמבין context ארוך ועושה refactoring (ארגון מחדש של קוד) בצורה חלקה. זו הבחירה שלי ל-[Claude Code](/claude-code) הראשי.",
        ...
        "Qwen 2.5 Coder 32B:free ו-Qwen 3 Coder של Alibaba Cloud, דרך OpenRouter — בין המודלים החינמיים הטובים ביותר לקוד. איטיים יותר ממודלים בתשלום ומוגבלים ב-rate limit ב-free tier, אבל בחינם מוחלט. אצלי (אלעד) ה-backup המרכזי הוא דווקא Gemini החינמי, אבל אלה אופציות מצוינות. אצלכם — אם אין Claude Pro/Max, זה יכול להיות הכלי הראשי. Qwen 3 Coder (הגרסה החדשה) זמין ב-tier בתשלום של OpenRouter במחיר תחרותי מאוד.",
        "DeepSeek V3 ו-DeepSeek Coder של DeepSeek — מצטיינים ביחס ביצועים/מחיר. DeepSeek V3 הוא מודל reasoning כללי מצוין, וב-free tier של OpenRouter זמין כ-`deepseek/deepseek-chat-v3:free`. חזקים במיוחד ב-Python, קצת פחות חדים ב-TypeScript.",
        "[Ollama](/guide/ollama) עם Qwen 2.5 Coder / Qwen 3 Coder / DeepSeek Coder / Llama 3 (של Meta) / Gemma 3 (של Google DeepMind) מקומיים — האופציה הפרטית ביותר שיש. הקוד לא יוצא מהמחשב, אין לוג אצל אף ספק ענן, וזה מושלם לתעשיות רגישות (רפואה, משפט, ביטחון). הדרישה היחידה: מחשב עם כרטיס מסך חזק (GPU — מעבד גרפי) עם לפחות 24GB VRAM למודלים של 32B. בלי GPU טוב זה איטי מאוד.",
        "הדגל `--weak-model` — פטנט חיסכון חכם. Aider משתמש במודל קטן ופשוט לכתיבת כותרות commit (ההודעה הקצרה שמתארת מה השתנה). אין סיבה לשרוף Sonnet יקר על 'fix: typo in login' — הגדירו `weak-model: anthropic/claude-haiku-4-5` (Haiku זול הרבה יותר: ~$1/M input, $5/M output) או מודל חינמי של OpenRouter ותחסכו 30-40% מהעלות הכוללת.",
      ...
      tips: [
        "הדגל `--edit-format diff` מתאים לרוב הפרויקטים — הוא שולח ל-LLM רק את השינוי עצמו, לא את כל הקובץ. לשינויים גדולים ומורכבים, `--edit-format udiff` עדיף (זה פורמט diff מתוחכם יותר). Sonnet 4.6 מטפל בשניהם הכי טוב מכל המודלים.",
        "לפני שאתם מפעילים בקשה גדולה ב-Aider, הריצו את הפקודה `/tokens` בתוך הסשן — זה מראה לכם כמה טוקנים כרגע ב-context ומהי ההוצאה המשוערת. בערך 30 אלף טוקנים זו שיחה רגילה, 100 אלף זה כבר יקר, ומעל 200 אלף זה הימור על הארנק.",
        "לבעיות פשוטות (typo, תיקון שורה בודדת, שינוי משתנה) — השתמשו ב-Claude Haiku 4.5 של Anthropic, ב-GPT-4.1 mini של OpenAI, ב-Gemini 2.5 Flash או ב-Qwen 2.5 Coder:free. אין טעם לשרוף את Claude Opus 4.5, המודל הכי יקר של Anthropic ($15/M input), על bug של חמש דקות.",
      ],
      codeExample: {
    ...
    },
    {
      ...
      difficulty: "intermediate",
      beginner:
        "אל תיבהלו מרשימת הפקודות — בפועל העבודה היומיומית עם Aider היא שיחה פשוטה כמו עם עוזר. אתם מצביעים על קובץ ('בוא נעבוד על זה'), מבקשים בעברית רגילה ('תוסיף בדיקה שהאימייל תקין'), והוא עורך, מראה לכם בדיוק מה השתנה, ושומר. הפקודות הקצרות שמתחילות בלוכסן הן רק קיצורי דרך — מספיק לזכור שבע-שמונה מהן והעבודה זורמת. הכי שימושית מכולן היא /undo: כפתור 'בטל' שמחזיר את הכל אחורה אם המודל פספס. אצלי (אלעד) זה מה שמאפשר לעבוד בלי פחד.",
      content: [
        ...
        "`/ls` — מראה רשימה של כל מה שנמצא כרגע ב-context: אילו קבצים נוספו, כמה טוקנים כל אחד תופס ואיזה מהם בסטטוס read-only (קריאה בלבד, לא ניתן לעריכה).",
        ...
        "בקשה בשפה חופשית — למשל 'תוסיף validation (בדיקת תקינות) לשדה האימייל בקובץ src/forms/signup.ts, כך שיחסום כתובות לא חוקיות'. Aider עורך את הקובץ, מראה diff, שואל אישור ועושה commit עם הודעה מפורטת.",
        ...
        "`/tokens` — כמה טוקנים כרגע ב-context ומהי העלות המשוערת של בקשה נוספת. מצוין כדי לדעת מתי לעשות `/clear` ולהתחיל session חדש.",
        "`/clear` — מנקה את היסטוריית השיחה (chat history) אבל שומר את הקבצים שהוספתם. שימושי כשעוברים ממשימה אחת לאחרת ולא רוצים ש-Aider 'יזכור' את מה שהיה קודם.",
        "`/exit` — יציאה נקייה. השינויים כבר שמורים ב-git, אז סגירת Aider לא פוגעת בעבודה.",
      ],
      tips: [
        "התחילו כל session עם `/add` של כל הקבצים הרלוונטיים לפני הבקשה הראשונה. context מקיף = תשובה טובה יותר. אל תוסיפו קבצים טיפה-טיפה — זה מבלבל את ה-LLM ואת עצמכם.",
        "`/model anthropic/claude-sonnet-4-6` מחליף את המודל באמצע session. שימושי כשהמודל החלש נכשל ואתם צריכים 'לשדרג' לעוצמה רצינית יותר — או להפך, כשסיימתם את החלק המורכב ורוצים לחזור למודל זול לפעולות קלות. פקודות נוספות שחוסכות זמן: `/architect` להדלקת architect mode, `/commit` ל-commit ידני ו-watch mode (`--watch-files`) שגורם ל-Aider לעקוב אוטומטית אחרי קבצים שאתם עורכים ב-editor אחר.",
    ...
    },
    {
      ...
      difficulty: "advanced",
      beginner:
        "עד כאן Aider היה כלי שמדברים איתו ביד; כאן הוא הופך לעובד שרץ לבד. היכולת הגדולה היא שאפשר לתת לו פקודה אחת בלי שום שיחה — 'תתקן את כל שגיאות הסגנון בקובץ הזה' — והוא עושה את זה ויוצא. זה פותח דלת לאוטומציה: למשל, בכל פעם שמישהו מציע שינוי קוד, Aider עובר עליו אוטומטית ומנקה אותו בלילה, בלי שאף אחד יושב מול המסך. אצלי (אלעד) הוא בעצם 'מנתח הקוד' של הרשת — רכיב שמקבל משימה מדויקת מסוכן אחר, מבצע אותה בזהירות עם רשתות ביטחון, ומחזיר את התוצאה לבדיקה. הסעיף הזה טכני יותר, אבל הרעיון פשוט: הכלי שעבד לידכם יכול גם לעבוד בשבילכם.",
      content: [
        ...
        "שערי-בטיחות: branch-only לעולם לא main · dry-run→אישור-פר-משימה · file-allowlist (אסור secrets/config/deploy) · תקרת-גודל-diff + rollback אוטומטי · לוגים גלויים. ושער-הכתיבה שלו סגור כברירת-מחדל בכוונה — נפתח ידנית רק כשיש משימה אמיתית. הג'ובים שלו: ריפקטור-מכני, דחיפת-תלויות, lint/type-fixes, יצירת-טסטים, ו-'מימוש-שני' (diff חלופי להשוואה).",
        ...
        "קלט קולי: `aider --voice-language he` דרך OpenAI Whisper — מאפשר להכתיב בקשות בעברית במקום להקליד. פי 2 מהיר למשפטים ארוכים, ובעברית עובד טוב במיוחד.",
        ...
        "CONVENTIONS.md: אחת התכונות השקטות והכי שימושיות של Aider. כתבו קובץ CONVENTIONS.md ב-root של הפרויקט עם 10-15 כללים ('אני משתמש ב-bun לא npm', 'תמיד TypeScript strict', 'נקודה-פסיק בסוף שורה'), ו-Aider יקרא אותו לפני כל בקשה וייצמד לכללים. זה מעין 'מדריך סגנון' שה-AI מכיר.",
        "Read-only context: הדגל `--read-only file.ts` אומר ל-Aider 'קרא את הקובץ להבנה, אבל אל תיגע בו'. שימושי כשרוצים שמשהו ישמש כהקשר (context) בלי להיות בסיכון להשתנות — למשל קבצי schema, constants או הגדרות בסיסיות.",
        "Linter integration: `--auto-lint` מריץ linter (כלי שבודק סגנון קוד) אוטומטית אחרי כל עריכה של Aider. אם ה-lint נכשל, Aider רואה את השגיאות ומתקן אותן בו במקום. ככה הקוד יוצא נקי כבר מההתחלה.",
      ],
      tips: [
        "הגדירו קובץ CONVENTIONS.md עם 10-15 כללי הזהב של הפרויקט — Aider עוקב אחריהם בכל עריכה. אצלי (אלעד) יש שם דברים כמו 'תמיד [better-result](/guide/claude-code) לטיפול בשגיאות, לא try/catch', 'bun לא npm', 'אלמנטים ב-Tailwind v4 בלבד'.",
        "ב-CI: תמיד שלבו `--yes` עם `--no-pretty` (שמונע צבעי ANSI בלוגים — הם יוצאים כמו 'ג'יבריש' בלוגים שאינם טרמינל). לפני פריסה אמיתית, הריצו את הזרימה פעם אחת על branch פרטי כדי לוודא שהפלט יוצא נקי.",
        "ב-monorepo: הריצו את Aider מתוך ה-package הבודד ולא מ-root של ה-monorepo. ככה ה-repo-map שלו הרבה יותר מדויק ולא מציף את ה-LLM ב-200 קבצים שלא רלוונטיים לבקשה הנוכחית.",
      ],
      codeExample: {
    ...
    },
    {
      ...
      description:
        "שניהם כלים מעולים, ובאמת לא מתחרים אלא משלימים אחד את השני. הבחירה תלויה בהקשר: מה המשימה, איזה תקציב יש, אם צריך פרטיות מוחלטת או שקיפות ענן. אצלי (אלעד) ההרכב הוא [Claude Code](/claude-code) כראשי לפיתוח רציני ויומיומי, ו-Aider ל-tasks מבודדים ולחיסכון במכסה של המנוי הראשי. אצלכם — הבחירה תלויה קודם כל בשאלה 'האם יש לי מנוי [Claude Max](/claude-code)?'. אם כן, Claude Code מוביל. אם לא, Aider לוקח את הבמה.",
      color: "from-emerald-600 to-teal-500",
      difficulty: "beginner",
      beginner:
        "השאלה הכי נפוצה היא 'Aider או Claude Code?', והתשובה הכנה היא: זה לא או-או, הם משלימים. כמו ההבדל בין רכב שטח מאובזר לאופנוע זריז — לכל אחד יש מצב שבו הוא עדיף. הכלל הפשוט שלי (אלעד): אם יש לכם מנוי [Claude Max](/claude-code), הוא הכלי הראשי לרוב העבודה, בזכות כל היכולות החכמות שבו. אם אין לכם, או שאתם צריכים לעבוד בחינם או על קוד רגיש שאסור שייצא מהמחשב — Aider לוקח את הבמה ועדיין נותן תוצאה ברמה גבוהה. אין כאן מנצח אחד; יש כלי נכון לכל מצב.",
      content: [
        ...
        "Aider הוא CLI טהור ומינימלי — פשוט יותר, מהיר יותר, קל יותר לאוטומציה. תומך ביותר מ-200 ספקי AI דרך LiteLLM (Anthropic, OpenAI, Google, xAI, Meta דרך OpenRouter, DeepSeek, Mistral AI, Cohere ועוד), יש לו מסלול חינמי מלא דרך OpenRouter, ותמיכה קהילתית ב-MCP (דרך גרסאות fork וקונפיגורציה ידנית). היתרון: מי שצריך backup או אין לו תקציב יקבל כלי שעדיין ברמה גבוהה.",
        ...
        "שילוב git: שני הכלים מעולים, אבל אצל Aider זה אוטומטי לחלוטין — כל שינוי הופך ל-commit ללא שאלות. ב-[Claude Code](/claude-code) נדרשת פקודת `/commit` מפורשת, מה שנותן שליטה אבל דורש יותר צעדים.",
        "Context רב-קבצי: [Claude Code](/claude-code) משתמש ב-sub-agents לפרויקטים גדולים מאוד (עשרות אלפי קבצים). Aider עם repo-map עובד יפה עד גודל בינוני-גדול, ואז ממליץ להתמקד בתת-פרויקט בודד.",
        ...
        "AI מקומי: Aider מצוין יחד עם [Ollama](/guide/ollama) ומאפשר עבודה air-gapped (מנותקת לגמרי מהאינטרנט — הקוד לא יוצא מהמחשב). [Claude Code](/claude-code) תלוי בשירות הענן של Anthropic, ונכון לעכשיו אין לו מצב offline.",
      ],
      tips: [
        ...
        "אם אין לכם [Claude Max](/claude-code) — התחילו עם Aider + OpenRouter Qwen 2.5 Coder:free או DeepSeek V3:free. תקבלו 80% מהיכולת בחינם. כשתתקלו בבקשה שבה Aider לא מספיק, שקלו מעבר ל-[Claude Code](/claude-code).",
      ],
    },
  ],
  resources: [
