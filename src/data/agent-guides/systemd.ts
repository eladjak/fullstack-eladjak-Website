...
export const systemdGuide: AgentGuideData = {
  ...
  sections: [
    ...
    },
    {
      ...
      difficulty: "beginner",
      beginner:
        "אם service unit הוא תעודת הזהות, systemctl הוא השלט שבו אתם מפעילים ומכבים. ארבע פקודות מכסות כמעט הכול: start (הדלק), stop (כבה), restart (כבה והדלק) ו-status (מה שלומך?). הכי שימושי מביניהם זה דווקא status — זאת הפקודה הראשונה שמריצים כשמשהו לא עובד, והיא גם אומרת אם השירות חי וגם מראה את שורות הלוג האחרונות שלו. כמו למדוד חום למישהו לפני שמחליטים מה לעשות.",
      content: [
    ...
    },
    {
      ...
      difficulty: "intermediate",
      beginner:
        "לוג זה היומן של התוכנה — כל מה שהיא 'אמרה' לעצמה תוך כדי ריצה: מה הצליח, מה נכשל, מתי. כשמשהו מתקלקל, היומן הזה הוא העדות הכי טובה שיש. היופי ב-journalctl הוא שהוא אוסף את היומנים של כל השירותים למקום אחד, ונותן לחפש בהם כמו בגוגל — 'תראה לי רק את השגיאות מהשעה האחרונה', 'תעקוב אחרי מה שקורה עכשיו בזמן אמת'. במקום לרדוף אחרי עשרה קבצים מפוזרים, הכול נשאל ממקום אחד.",
      content: [
    ...
    },
    {
      ...
      difficulty: "intermediate",
      beginner:
        "Timer זה שעון מעורר לשרת — אומרים לו 'תעשה את הדבר הזה כל לילה ב-3', והוא דואג שזה יקרה. פעם זה נעשה עם כלי ותיק בשם cron, ש-systemd-timers בא להחליף. היתרון הגדול: אם השרת היה כבוי בדיוק בשעה שהמשימה הייתה אמורה לרוץ, timer 'מתעורר' ומריץ אותה ברגע שהוא חוזר לעצמו — בעוד cron פשוט מפספס בלי לספר לכם. בנוסף נשמרת היסטוריה מלאה, אז תמיד אפשר לבדוק אם הגיבוי של אתמול בלילה באמת רץ.",
      content: [
    ...
    },
    {
      ...
      difficulty: "advanced",
      beginner:
        "כאן systemd עושה משהו מפתיע: הוא יכול לשים כל שירות ב'חדר משלו'. לתת לו תקרת זיכרון שאסור לחרוג ממנה, לחסום אותו מלגעת בקבצים שלא שייכים לו, ולוודא שגם אם תהיה בו פרצת אבטחה — הנזק נשאר כלוא באותו חדר ולא מתפשט לכל השרת. הרבה אנשים חושבים שצריך Docker בשביל הבידוד הזה, אבל לפרויקטים פשוטים systemd נותן בערך 80% מזה בכמה שורות, בלי לבנות image ובלי השכבה הנוספת.",
      content: [
