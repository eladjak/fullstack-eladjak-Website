...
export const ollamaGuide: AgentGuideData = {
  ...
  sections: [
    {
      ...
      content: [
        ...
        "ממשק API זהה לחלוטין לזה של OpenAI, כך שכל תוכנה שכבר מדברת עם ChatGPT יכולה לדבר עם Ollama במקומו, בהחלפת כתובת אחת",
        ...
        "המודלים מגיעים בפורמט חסכוני (GGUF) שדוחס את המידע מבלי לפגוע משמעותית באיכות, כך שגם מחשב ביתי עם 8GB RAM יכול להריץ מודל מכובד",
      ],
      tips: [
        "התחילו מ-Gemma 3 בגודל 4B: ברירת מחדל מצוינת שרצה יפה גם על MacBook Air רגיל, ונחשבת למודל החזק ביותר בגודלה נכון ל-2026. איכות מצוינת, מהיר ודורש כ-3.3GB זיכרון בלבד.",
        ...
        "לעבודה על קוד, Qwen 3 Coder (בגודל 30B) ו-DeepSeek-Coder-V2 עדיפים משמעותית על מודלי שיחה כלליים. הם אומנו על מיליארדי שורות קוד אמיתיות ומחזירים פלט מדויק יותר.",
      ],
      codeExample: {
        label: "שתי פקודות, ויש לכם AI מקומי",
    ...
    },
    {
      ...
      description:
        "התקנת Ollama היא פעולה פשוטה מאוד שנתמכת בכל מערכות ההפעלה הנפוצות. ההמלצה שלי: התקנה ישירה על המחשב (Mac ו-Linux) שנותנת גישה מיידית לכרטיס המסך (GPU) שלכם ומאיצה את הביצועים משמעותית. [Docker](/guide/docker), מערכת שמריצה תוכנות בתוך 'קופסאות' מבודדות, שמור למי שבאמת צריך הפרדה בין שרתים או עובד בסביבת ייצור (production).",
      color: "from-emerald-600 to-teal-500",
      difficulty: "beginner",
      beginner:
        "אם יש לכם Mac מודרני (עם שבב M1/M2/M3) או מחשב Linux עם כרטיס NVIDIA, ההתקנה היא סקריפט אחד שרץ בטרמינל ותוך כמה דקות הכל מוכן. ב-Windows האפשרות הפשוטה היא להוריד את Ollama Desktop (גרסה ויזואלית עם אייקון בשורת המשימות), ולשרתים המקצועיים מומלץ להשתמש ב-Docker.",
      content: [
        "ב-Mac: פקודה אחת בטרמינל (curl | sh) או הורדת האפליקציה הרשמית מ-ollama.com, מקבלים אייקון נחמד בשורת התפריטים העליונה",
        ...
        "ב-[Docker](/guide/docker): יש image רשמי (ollama/ollama:latest) שרץ עם הדגל --gpus=all אם יש NVIDIA, או בלי הדגל הזה לריצה על מעבד בלבד",
        "כונן SSD מהיר חשוב מאוד: כשמפעילים מודל הוא נטען מהדיסק לזיכרון, וכונן NVMe מודרני מוריד את זמן ההמתנה הראשוני בחצי",
      ...
      tips: [
        ...
        "ב-Linux, הדרך המסודרת להוסיף הגדרות היא דרך sudo systemctl edit ollama, כך אפשר להוסיף משתני סביבה מבלי לשבור את העדכונים הבאים",
    ...
    },
    {
      ...
      beginner:
        "חשבו על שני מימדים פשוטים לבחירה: (1) כמה זיכרון יש לכם, הן רגיל (RAM) והן בכרטיס המסך (VRAM, הזיכרון של ה-GPU); (2) מהי המשימה העיקרית, שיחה רגילה, כתיבת קוד, סיווג טקסטים (classification) או יצירת embeddings (המרת טקסט לייצוג מספרי שמאפשר השוואות חכמות). ההצטלבות של שני המימדים האלה תיתן לכם את המודל המתאים.",
      content: [
        "למשימות כלליות (שיחה, סיכומים, תרגום, כתיבה יצירתית): Gemma 3 בגודל 4B מהיר וחסכוני לעבודה יומיומית / Llama 3.1 בגודל 8B אלטרנטיבה מצוינת עם הקשר של 128k טוקנים / Qwen 3 בגודל 14B הוא נקודת המתיקות שבין איכות לדרישות חומרה / Llama 3.3 בגודל 70B ו-Qwen 3 בגודל 32B מביאים איכות ברמת מודלי ענן, אך דורשים חומרה חזקה (32–48GB VRAM)",
        "למשימות חשיבה מורכבות (reasoning, פתרון בעיות מתמטיות, לוגיות, ניתוחים רב-שלביים): DeepSeek-R1 (זמין במספר גדלים מ-1.5B עד 70B), Qwen 3 במצב thinking-mode, או gpt-oss של OpenAI (גרסה בקוד פתוח של GPT). אלה מודלים חדשים שמבצעים 'חשיבה פנימית' לפני שמחזירים תשובה",
      ...
      tips: [
        "ברירת המחדל המומלצת ל-quantization (דחיסה חכמה של המודל שמצמצמת משקל מבלי לפגוע משמעותית באיכות) היא Q4_K_M. גרסת Q8_0 נותנת איכות מעט גבוהה יותר אבל דורשת פי 2 זיכרון, בדרך כלל לא שווה את זה לשימוש יומיומי",
    ...
    },
    {
      ...
      difficulty: "intermediate",
      beginner:
        "API זה פשוט הדרך שבה תוכנות אחרות 'מדברות' עם Ollama מתוך הקוד, בלי שבן אדם יקליד. והקסם הגדול כאן: ה-API של Ollama בנוי להיות זהה לזה של ChatGPT. למה זה כל כך חשוב? כי זה אומר שכל תוכנה שכבר עובדת עם ChatGPT יכולה לעבור למודל מקומי וחינמי תוך שינוי של שורה אחת, כמו להחליף את הספק בלי להחליף את כל הצנרת בבית. אצלכם זה אומר שאפשר לבנות אפליקציה מול ChatGPT, ובהמשך להעביר אותה לרוץ חינם על המחשב שלכם בלי לכתוב הכל מחדש.",
      content: [
        ...
        "החלפה שקופה ל-OpenAI: משנים את כתובת ה-API ל-http://localhost:11434/v1, נותנים מפתח API דמה (כל מחרוזת מזהה בסיסית), וזה עובד מיידית עם LangChain, LlamaIndex, Vercel AI SDK וכל לקוח אחר",
      ...
      tips: [
        ...
        "למשימות שדורשות פלט מובנה (כמו מילוי טופס): הפרמטר response_format: 'json' יאלץ את Gemma 3 להחזיר JSON חוקי, מצוין לאוטומציות ב-[CrewAI](/guide/crewai) או ב-[n8n](/guide/n8n)",
      ],
      codeExample: {
    ...
    },
    {
      ...
      difficulty: "advanced",
      beginner:
        "השאלה הראשונה שכולם שואלים היא 'כמה מהר זה ירוץ אצלי?', והתשובה הכנה: תלוי. בדיוק כמו מכונית, היא נוסעת מהר יותר עם מנוע חזק ועם פחות משקל. כאן ה'מנוע' הוא כרטיס המסך (GPU) שבמחשב, וה'משקל' הוא גודל המודל. מודל קטן על מחשב חזק עף; מודל ענק על מחשב רגיל יזחל. הכלל הפשוט: התחילו ממודל קטן, ותגדילו רק אם המחשב שלכם עומד בזה בנוחות. הסעיף הזה נותן מספרים מציאותיים לכל סוג מחשב, כדי שתדעו מראש למה לצפות במקום להתאכזב.",
      content: [
        ...
        "זמן ההמתנה לטוקן הראשון (first-token latency) תלוי בעיקר בזמן הטעינה של המודל לזיכרון. ההגדרה keep_alive='10m' מונעת טעינה חוזרת בין בקשות ומקצרת משמעותית את התגובה",
        "במקביליות (concurrency): Ollama מריץ כמות מוגבלת של בקשות במקביל. לצרכי תפוקה גבוהה (throughput) עדיף להשתמש ב-vLLM או ב-TGI, שרתי inference מקצועיים יותר שבנויים לעומסים גדולים",
      ...
      tips: [
        ...
        "להשוואת ביצועים מקצועית יש כלי בשם llama-bench (כלול ברפוזיטורי של Ollama), הוא הרבה יותר מדויק מהרצת שאילתות ידנית ומדידה בשעון יד",
      ],
    },
    {
      id: "integration",
      ...
      description:
        "אינטגרציה היא הנקודה שבה Ollama הופך מכלי מקומי נחמד לחלק פועם של מערכת רחבה יותר. בקוד של רשת הסוכנים שלי קיימת שכבת fallback (תוכנית גיבוי) כזו, היא יודעת לעבור למודל מקומי כשהענן לא זמין. גילוי נאות: כרגע השכבה הזו לא פעילה בשרת עצמו (Ollama מותקנת אצלי רק על תחנת העבודה), אבל הדוגמאות כאן מראות בדיוק איך מחברים אותה. בזכות ה-endpoint תואם-OpenAI, כל מודל ברשת יכול לעבור מ-Claude או מ-Gemini ל-Ollama בשינוי כתובת בלבד, שימושי במיוחד למשימות classification בתוך [Adopter](/guide/adopter) ולסיווג של intakes ב-[Box](/guide/box).",
      color: "from-rose-600 to-pink-500",
      difficulty: "advanced",
      beginner:
        "כאן Ollama מפסיק להיות צעצוע מקומי נחמד והופך לחלק עובד במערכת אמיתית. התפקיד המיועד לו אצלי (אלעד) כפול: רשת ביטחון (אם המודל בענן עמוס או עולה כסף, המשימה נופלת בעדינות למודל המקומי החינמי), ועובד שקט ברקע למשימות פשוטות שחבל לשלם עליהן לענן, כמו למיין פוסטים או לתייג נתונים. היום Ollama רצה אצלי על תחנת העבודה, והשכבה בקוד השרת מחכה להפעלה. כיוון שה-API שלו זהה לזה של ChatGPT, כל סוכן ברשת יכול לעבור אליו בשינוי כתובת בלבד. הרעיון: לשמור את המודלים היקרים למשימות שבאמת צריכות אותם, ולתת לכל השאר לרוץ חינם.",
      content: [
        ...
        "[CrewAI](/guide/crewai): Agent(llm=LLM(model='ollama/qwen2.5:7b')), תמיכה מובנית וטבעית, ללא אדפטרים או wrappers",
        "Fallback ב-[Kami](/guide/kami): בקוד קיימת שכבה שאם Claude מחזיר שגיאה (429 זה חריגת quota, 500 זה תקלת שרת), היא תופסת את החריגה ועוברת למודל מקומי, והשיחה ממשיכה בלי שהמשתמש הרגיש. אצלי היא כתובה ומוכנה, אבל לא פעילה בשרת כל עוד Ollama לא מותקנת שם",
        "משימות סיווג המוניות, כמו אלה של [Adopter](/guide/adopter), הן מועמדות טבעיות למודל מקומי, במקום לשרוף quota בענן. אצלי הסיווג רץ היום על השכבה החינמית של Gemini, אבל זה בדיוק סוג המשימה ש-Ollama יכולה לקחת",
        "Embeddings: המודל nomic-embed-text רץ ב-Ollama בכ-150 טוקנים לשנייה ומהווה חלופה מעולה ל-embeddings של Gemini (שחינמיים אבל עם מגבלת קצב של דקה-לדקה)",
        "במעקב העלויות (/costs/llm) הקריאות מסומנות כ-'local', המשמעות בפועל היא עלות אפסית, סופרים רק את החשמל של המחשב",
      ],
      tips: [
        "תמיד בדקו את המודל על הדאטה האמיתי שלכם. מודל שמקבל ציון גבוה על benchmarks סטנדרטיים (כמו MT-Bench) לא בהכרח מצטיין בעברית או בדומיין הספציפי שלכם",
        "גישה היברידית: [Claude](/claude-code) לחשיבה מורכבת ולמקרים קריטיים, Ollama לסיווג מסיבי של נתונים ברקע. זה השילוב החסכוני והיציב ביותר שמצאתי",
  ...
  resources: [
    ...
    {
      title: "llama.cpp",
      description: "ה-engine שמתחת. שימושי להבנת GGUF ו-quantization",
