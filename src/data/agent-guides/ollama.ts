...
export const ollamaGuide: AgentGuideData = {
  ...
  sections: [
    ...
    },
    {
      ...
      description:
        "אינטגרציה היא הנקודה שבה Ollama הופך מכלי מקומי נחמד לחלק פועם של מערכת רחבה יותר. ברשת הסוכנים שלי, Ollama ממלא תפקיד של רשת ביטחון (fallback — תוכנית גיבוי) וגם של עובד ברקע למשימות שגרתיות שלא מצדיקות תשלום לענן. בזכות ה-endpoint תואם-OpenAI, כל מודל ברשת יכול לעבור מ-Claude או מ-Gemini ל-Ollama בשינוי כתובת בלבד. זה שימושי במיוחד למשימות classification בתוך [Adopter](/guide/adopter) ולסיווג של intakes ב-[Box](/guide/box).",
      color: "from-rose-600 to-pink-500",
      difficulty: "advanced",
      beginner:
        "כאן Ollama מפסיק להיות צעצוע מקומי נחמד והופך לחלק עובד במערכת אמיתית. אצלי (אלעד) הוא ממלא שני תפקידים: רשת ביטחון (אם המודל בענן עמוס או עולה כסף, המשימה נופלת בעדינות למודל המקומי החינמי), ועובד שקט ברקע למשימות פשוטות שחבל לשלם עליהן לענן — כמו למיין פוסטים או לתייג נתונים. כיוון שה-API שלו זהה לזה של ChatGPT, כל סוכן ברשת יכול לעבור אליו בשינוי כתובת בלבד. הרעיון: לשמור את המודלים היקרים למשימות שבאמת צריכות אותם, ולתת לכל השאר לרוץ חינם.",
      content: [
        ...
        "Fallback ב-[Kami](/guide/kami): אם Claude מחזיר שגיאה (429 זה חריגת quota, 500 זה תקלת שרת) — הקוד תופס את החריגה ועובר ל-Ollama אוטומטית, והשיחה ממשיכה בלי שהמשתמש הרגיש",
