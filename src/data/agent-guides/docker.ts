...
export const dockerGuide: AgentGuideData = {
  ...
  paradigmSub:
    "בלי containers, להריץ 10 שירותים שונים על שרת אחד הוא סיוט של תלויות. עם docker-compose, זה קובץ YAML אחד.",
  paradigmShifts: [
    {
  ...
  sections: [
    ...
    },
    {
      ...
      beginner:
        "אצלי (אלעד) ב-compose.yml יש את כל רשת הסוכנים: [Kami](/guide/kami), [Kaylee](/guide/kaylee), [Qdrant](/guide/qdrant), [Delegator](/guide/delegator), [Hermes](/guide/hermes) ועוד. במקום 10 פקודות נפרדות, יש קובץ אחד. משנים הגדרה בקובץ, מריצים `docker compose up -d` — ורק מה שבאמת השתנה מתעדכן, כל השאר ממשיך לרוץ בלי הפסקה. זו חוויית פיתוח שמאפשרת לבנות מערכות רציניות בלי כאב ראש תפעולי.",
      content: [
    ...
    },
    {
      ...
      tips: [
        "אל תחשפו פורטים סתם — חשפו רק את מה שחייב להיות נגיש מבחוץ. אצלי (אלעד) רק ה-[Delegator](/guide/delegator) שמשמש כ-reverse proxy (שרת קדמי שמקבל את כל התעבורה ומנתב פנימה) ו-nginx חשופים לעולם",
    ...
    },
    {
      ...
      content: [
        ...
        "אצלי (אלעד) כל ה-bind mounts מרוכזים תחת `/opt/ai-factory/data/` — מבנה אחיד שקל לגבות ולהעביר בין שרתים בפעולה אחת",
      ],
      tips: [
  ...
  authorBio:
    "כל רשת הסוכנים שלי (10 שירותים, 14 containers, Qdrant עם אלפי וקטורים) רצה ב-docker-compose יחיד על Hetzner CPX11 בכ-4.75€/חודש. המדריך הזה מבוסס על הניסיון של הקמת הרשת הזו מחדש 3 פעמים במהלך שנתיים, כולל migration מ-ARM ל-x86 ושחזור אחרי crash של disk.",
};
