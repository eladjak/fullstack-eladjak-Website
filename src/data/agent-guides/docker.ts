...
export const dockerGuide: AgentGuideData = {
  ...
  stats: [
    { label: "containers אצלי", value: "~27" },
    { label: "compose stacks", value: "~10" },
    { label: "שירותי systemd לצידם", value: "~45" },
    { label: "uptime", value: "99.7%" },
  ],
  paradigmTitle: "למה Docker הוא הבסיס של כל הרשת",
  paradigmSub:
    "בלי containers, להריץ עשרות שירותים שונים על שרת אחד הוא סיוט של תלויות. עם docker-compose, זה כמה קבצי YAML מסודרים.",
  paradigmShifts: [
    {
  ...
  sections: [
    {
      ...
      content: [
        ...
        "Image (אימג') — התבנית שממנה מריצים קונטיינר. כמו שרטוט של בית שממנו אפשר לבנות אינסוף בתים זהים (אותו יחס שיש בין מחלקה בתכנות לעצם שנוצר ממנה)",
        ...
        "Docker Hub — הספרייה הציבורית של העולם לאימג'ים מוכנים (nginx, postgres, python ועוד אלפים). שורה אחת בקוד ויש לכם שרת דאטאבייס פועל",
      ...
      tips: [
        "משתמשי Mac — OrbStack (8$ לחודש אישי / 18$ לחודש עסקי) חוסך כ-50% מצריכת הזיכרון לעומת Docker Desktop ומהיר יותר באופן מורגש; Colima היא אלטרנטיבה חינמית לגמרי בקוד פתוח למי שמעדיף את זה",
        "`docker ps` מראה אילו קונטיינרים רצים כרגע; `docker logs -f <name>` מציג את הפלט שלהם בזמן אמת — שתי פקודות שתשתמשו בהן כל יום",
      ],
      codeExample: {
    ...
    },
    {
      ...
      beginner:
        "אצלי (אלעד) הרשת מחולקת לכמה קבצי compose — אחד לכל תחום: [Qdrant](/guide/qdrant) והזיכרון, [n8n](/guide/n8n) והאוטומציות, [Kaylee](/guide/kaylee), ניטור ועוד (סוכנים אחרים, כמו [Kami](/guide/kami), רצים דווקא כשירותי systemd — לכל כלי יש את המקום הנכון שלו). במקום עשרות פקודות נפרדות, יש קובץ אחד לכל תחום. משנים הגדרה בקובץ, מריצים `docker compose up -d` — ורק מה שבאמת השתנה מתעדכן, כל השאר ממשיך לרוץ בלי הפסקה. זו חוויית פיתוח שמאפשרת לבנות מערכות רציניות בלי כאב ראש תפעולי.",
      content: [
        ...
        "volumes (אחסון מתמיד) — מוגדרים ברמת ה-compose, משותפים בין שירותים ושורדים אתחול של הקונטיינר. שם נשמרת כל הדאטה שחייבת להישאר",
        "env_file — מצביעים על קובץ `.env` והוא נטען אוטומטית לכל השירותים. אפשר לדרוס אותו לכל שירות בנפרד כשצריך",
        ...
        "healthcheck (בדיקת בריאות) — Docker בודק כל כמה שניות אם הקונטיינר 'חי ומגיב'. אפשר לחבר את זה ל-`depends_on` כך שהשירות הבא יתחיל רק אחרי שקודמו באמת מוכן (ולא רק 'רץ')",
      ],
      tips: [
        "שמרו את `compose.yml` בתוך גיט, אבל השאירו את קובץ ה-`.env` מחוץ לגיט (הוסיפו אותו ל-`.gitignore`) — הוא מכיל סודות",
        "הוסיפו `restart: unless-stopped` לכל שירות. ככה אחרי אתחול של השרת (תקלת חשמל, עדכון מערכת) הכל יחזור לעבוד לבד, ללא התערבות",
    ...
    },
    {
      ...
      difficulty: "intermediate",
      beginner:
        "חשבו על זה ככה: כשמקימים compose, Docker בונה סביב כל השירותים 'חצר פנימית' סגורה. בתוך החצר, כולם מדברים בחופשיות ומכירים זה את זה לפי שם. החוצה — לאינטרנט הפתוח — לא חשוף שום דבר, אלא אם אתם מבקשים זאת במפורש באמצעות פתיחת פורט. זה מקנה לכם ביטחון מובנה: טעות בהגדרה לא חושפת אוטומטית את בסיס הנתונים שלכם לעולם.",
      content: [
        "Bridge network (רשת גשר) — ברירת המחדל של compose. רשת פרטית אחת שמחברת את כל השירותים שמוגדרים באותו קובץ, ללא הגדרה נוספת",
        "Service Discovery — שם השירות שהגדרתם ב-compose הופך אוטומטית לכתובת ברשת. `http://kami:3001` יעבוד מתוך כל קונטיינר אחר ברשת",
        "Ports (פורטים) — ההגדרה `X:Y` אומרת 'תפתח את הפורט Y הפנימי דרך הפורט X החיצוני'. חשוב להבין: עושים את זה רק כשרוצים גישה מבחוץ לשירות — ברוב המקרים שירותים פנימיים לא צריכים להיחשף כלל",
      ...
      tips: [
        "אל תחשפו פורטים סתם — חשפו רק את מה שחייב להיות נגיש מבחוץ. אצלי (אלעד) שום סוכן לא חשוף ישירות לאינטרנט: Traefik יושב בקצה כ-reverse proxy (שרת קדמי שמקבל את כל התעבורה ומנתב פנימה), והתעבורה מגיעה אליו דרך [Cloudflare Tunnel](/guide/cloudflare-tunnel)",
        "לאבחון בעיות רשת — `docker exec -it <name> ping <other-service>` יבדוק אם שירות א' רואה את שירות ב'. זה הצעד הראשון כשמשהו לא מתחבר",
        "Caddy או Traefik כ-reverse proxy חוסכים המון כאב — הם מתחברים ל-Docker labels ומייצרים תעודות HTTPS אוטומטית מ-Let's Encrypt, בלי התעסקות ידנית עם nginx",
      ],
    },
    {
      id: "volumes",
      ...
      difficulty: "intermediate",
      beginner:
        "בלי Volume, כל קונטיינר שמתחיל הוא כאילו 'מעולם לא היה' — זיכרון ריק, אפס היסטוריה. עם Volume, Docker שומר תוכן של תיקיות מסוימות מחוץ לקונטיינר, על השרת עצמו. ככה [Qdrant](/guide/qdrant) זוכר את כל הזיכרון שצבר [Kami](/guide/kami) גם אחרי שהשרת אותחל, וכל השיחות שעברתי איתו לא נמחקות.",
      content: [
        ...
        "tmpfs — אחסון בזיכרון בלבד (RAM). נמחק בכל אתחול, אבל מהיר מאוד. שימושי למטמון (cache) זמני של דברים שאין צורך לזכור",
        ...
        "הרשאות — חשוב לדעת: בלי התאמת מזהי משתמש (UID), קבצים שהקונטיינר ייצור ב-bind mount יופיעו בשרת בבעלות root. תכננו את זה מראש כדי לא להיתקע",
        "אצלי (אלעד) כל ה-bind mounts מרוכזים תחת `/opt/` בתיקייה ייעודית לכל שירות — מבנה אחיד שקל לגבות ולהעביר בין שרתים בפעולה אחת",
      ],
      tips: [
        "השתמשו ב-`:ro` (read-only, קריאה בלבד) בכל פעם שקונטיינר רק צריך לקרוא ולא לכתוב — מגן מפני שגיאות של דריסה בטעות",
    ...
    },
    {
      ...
      difficulty: "advanced",
      beginner:
        "ההבדל בין פיתוח לייצור: בפיתוח אפשר להריץ קונטיינר בהרשאות על (root), לחשוף כל פורט ולא להסתכל על לוגים. בייצור כל אחד מהדברים האלה יכול להפיל את השירות או לחשוף דאטה של משתמשים. הצד הטוב הוא שיש צ'קליסט ברור — אחרי שעוברים עליו פעם אחת, זה הופך לרפלקס.",
      content: [
        "משתמש לא-root — הוסיפו `USER node` או `USER appuser` ב-Dockerfile. רוב האימג'ים הרשמיים (node, python וכו') כבר מגיעים עם משתמש מוכן. בלי זה, פריצה לקונטיינר = הרשאות על בתוכו",
        "ניהול סודות — השתמשו ב-Docker secrets או ב-Vault. אסור לשים קובץ `.env` בגיט, אפילו לא לשנייה אחת (הוא יישאר בהיסטוריה וסורקים אוטומטיים ימצאו אותו תוך דקות)",
        "מגבלות משאבים — תחת `deploy.resources.limits` קבעו `memory` ו-`cpus` לכל שירות. זה מונע מקונטיינר שפועל מחוץ לנורמה (דליפת זיכרון, תקלה) להפיל את כל השרת",
        ...
        "Reverse proxy — שרת קדמי שמקבל את כל התעבורה ומנתב פנימה. Caddy מתקין תעודות HTTPS אוטומטית מ-Let's Encrypt ללא הגדרה; Traefik עושה את זה דרך Docker labels",
      ...
      tips: [
        "אל תחשפו את ה-Docker socket דרך HTTP בשום מקרה — זו מסירת מפתחות הממלכה. לניהול מרחוק השתמשו ב-Portainer עם אימות והצפנה מלאה (TLS)",
        ...
        "אל תעלו בשום אופן אימג' ל-registry ציבורי עם מפתחות API מובנים בתוכו. יש סורקים אוטומטיים שעוברים על Docker Hub כל הזמן ומחפשים בדיוק את זה — הם ימצאו לפני שתספיקו למצמץ",
      ],
    },
    {
      id: "debugging",
      ...
      difficulty: "intermediate",
      beginner:
        "כשקונטיינר לא עולה, אל תנחשו — תסתכלו. כמעט תמיד הבעיה היא לא תקלה ב-Docker עצמה אלא טעות הגדרה קטנה, וכמעט תמיד היומן (log) של הקונטיינר מספר בדיוק מה השתבש. אצלי (אלעד) יש שגרת בדיקה קבועה שמתחילה תמיד מאותה פקודה — 'תראה לי מה הקונטיינר אומר על עצמו' — וממשיכה הלאה רק אם צריך. זה כמו רופא שמודד חום לפני שמחליט על טיפול. הסעיף הזה הוא בעצם הסדר הזה: מה לבדוק קודם, מה אחר כך, עד שמוצאים את האשם.",
      content: [
        ...
        "`docker inspect <container>` — מחזיר את כל ההגדרות של הקונטיינר ב-JSON: הגדרות רשת, אילו נפחים מחוברים, אילו משתני סביבה יש. מושלם כשיש חשד שההגדרה לא תואמת למה שחשבתם",
        ...
        "dive — כלי חיצוני שמציג את האימג' שכבה-אחר-שכבה (layer-by-layer) ומגלה מה אפשר לחתוך. אימג' של Node יכול לצנוח מ-1GB ל-200MB כשיודעים איפה לחתוך",
        "`docker system df` — מראה כמה מקום על הדיסק תופסים אימג'ים, נפחים ומטמון בנייה. מועיל כשהשרת מתחיל להיות מלא",
      ],
      tips: [
        "אם קונטיינר נופל מיד אחרי שהוא עולה — `docker logs` הוא הצעד הראשון, תמיד. ב-99% מהמקרים ההודעה שמסבירה בדיוק את הבעיה תהיה בשורות האחרונות",
        "`docker compose config` מריץ בדיקת תקינות של קובץ ה-YAML ומציג איך הקומפוז באמת ייראה אחרי טעינת משתני הסביבה. מושלם לתפיסת טעויות תחביר לפני ההרצה",
        "אחרי שבוע של ניסויים — `docker system prune -a --volumes` מנקה את כל מה שלא בשימוש (אימג'ים, נפחים, רשתות זנוחות). זהירות: זה מוחק לצמיתות, אז הריצו רק אחרי שווידאתם שאין גיבוי שצריך",
      ],
    },
  ],
  resources: [
    ...
    },
    {
      ...
      description: "מאגר דוגמאות של compose files לכל סוג שירות",
      href: "https://github.com/docker/awesome-compose",
      icon: Github,
    },
    {
      title: "OrbStack",
      description: "אלטרנטיבה ל-Docker Desktop — קל ומהיר ב-Mac",
  ...
  authorBio:
    "רשת הסוכנים שלי רצה היום בארכיטקטורה היברידית על שרת Contabo: כ-27 קונטיינרים על כ-10 קבצי docker-compose, לצד כ-45 שירותי systemd, עם Traefik ו-Cloudflare Tunnel בקצה. היא התחילה על שרת Hetzner קטן והוקמה מחדש כמה פעמים בדרך — כולל הגירת שרתים מלאה. המדריך הזה מבוסס על הניסיון הזה.",
};
