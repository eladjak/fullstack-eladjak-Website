...
export const delegatorGuide: AgentGuideData = {
  ...
  heroDescription: "Delegator זה מה שנקרא בעגה המקצועית 'שער API' (API Gateway) — מעין מרכזייה דיגיטלית שיושבת בכניסה לרשת סוכני ה-AI שלי וממיינת את כל התנועה שעוברת דרכה. תחשבו על מרכזייה של בניין משרדים גדול: במקום שכל עובד יזכור את המספר של הבנק, של חברת הדואר, של ספק החשמל ושל עשרות הלקוחות — יש מרכזנית אחת שיודעת את הכל, העובד רק מבקש 'תעבירי אותי לבנק' והיא מחברת. בדיוק כך Delegator מתפקד עבור הסוכנים שלי: כש-[Kami](/guide/kami) רוצה לשלוח אימייל, כש-[Box](/guide/box) צריך ליצור אירוע ביומן, כש-[Adopter](/guide/adopter) רוצה לשמור משהו בזיכרון — אף אחד מהם לא צריך לדעת את מפתחות ה-API של Resend, Google, Twilio או Perplexity. הם פשוט שולחים בקשה ל-Delegator, והוא דואג לכל השאר. הטכנולוגיה עצמה פשוטה מאוד — שרת HTTP שכתבתי ב-Python עם מעל 100 נקודות קצה (endpoints) שמטפלות בכל מה שצריך. אצלי Delegator מחבר 10 סוכנים שונים מאחורי שער אחד — אצלכם הוא יכול להחליף שירותי תשלום כמו Zapier ו-Make בעשרות אלפי שקלים בשנה, ולשמש כתשתית API מרכזית לכל ארכיטקטורה עם מספר מערכות AI — בלי לפזר סיסמאות וקונפיגורציה בחמישה מקומות שונים.",
  ...
  sections: [
    {
      ...
      content: [
        ...
        "ניהול סודות (secrets) מרוכז — מפתחות ה-API של Resend (שירות שליחת מיילים), Twilio (SMS), Google OAuth, Anthropic, OpenAI, Gemini — כולם ב-environment variables של השרת, אף פעם לא בקוד ולא ב-Git",
        ...
        "אצלי: מרכז את 10 הסוכנים מאחורי שער אחד. אצלכם: יכול לשמש כ-gateway לרשת multi-agent משלכם, או להחליף Zapier/Make כשהם נהיים יקרים מדי",
      ],
    },
    {
      id: "routes",
      ...
      description:
        "endpoint זו כתובת ספציפית בשרת שעונה למשימה מסוימת. למשל POST /email/send לשליחת מייל, או GET /calendar/check לבדיקת לו\"ז. Delegator מחזיק מעל 100 כאלה, מחולקים לפי תחומי אחריות. זה לא רק middleware (שכבת תיווך שמעבירה בקשות לשירות אחר) — יש כאן גם לוגיקה עסקית אמיתית, למשל שער שבודק אם מותר לפרסם עכשיו, או router שמחליט איזה LLM להפעיל לפי עלות.",
    ...
    },
    {
      ...
      content: [
        ...
        "שמירת מצב במטמון (Caching): נתוני Hebcal (לוח שנה עברי), tokens של Google OAuth ונתונים אחרים שלא משתנים תכופות נשמרים בזיכרון למשך זמן מוגדר, בלי צורך לקרוא לכל API מחדש",
        "Feature flags (מתגי פיצ'רים): רוצים לכבות endpoint מסוים זמנית, למשל בזמן תחזוקה? שורה אחת של קוד — כולם מושפעים מיד",
      ],
      tips: [
        "טיפ שלמדתי בדרך הקשה: גם כש-Delegator וסוכן יושבים על אותו שרת פיזי, הסוכן עדיין פונה אליו דרך HTTP (ולא מייבא אותו כספרייה פנימית). זה נראה כמו בזבוז של 2ms, אבל זה מקטין דרמטית את ה-coupling (תלות הדוקה בין רכיבים). מחר תרצו להעביר את הסוכן לשרת אחר — לא תצטרכו לשנות שורת קוד",
      ],
    },
    {
      id: "calendar",
      ...
      content: [
        ...
        "שלוש קטגוריות רגישות: `shabbat_chag` (שבת וחגי ישראל — חסימה חזקה), `solemn_modern` (יום הזיכרון, יום השואה — חסימה של תוכן שיווקי), `celebratory_modern` (יום העצמאות — תוכן חגיגי מותר, שיווקי אגרסיבי לא)",
    ...
    },
    {
      ...
      description:
        "קריאות ל-LLM (מודלי שפה כמו Claude, Gemini, GPT) הן ההוצאה הכי גדולה ברשת סוכנים אוטונומית, ובלי מעקב זה יכול להתפוצץ לחשבון דמיוני בסוף החודש. הוספתי ל-Delegator מנגנון שמתעד כל קריאת LLM שעוברת דרכו: איזה tier (שכבה) השתמש, איזה model, מי הסוכן שביקש, כמה tokens נכנסו, כמה יצאו, וכמה זמן זה לקח. אפשר אחר כך לחתוך את הנתונים ולראות למשל 'Kami צרך 40% מתקציב ה-LLM החודשי'.",
      ...
      content: [
        ...
        "הנתונים נרשמים בפורמט JSONL (שורה JSON אחת לכל אירוע) בקובץ `/opt/ai-factory/data/costs/llm-calls.jsonl`. פורמט פשוט שקל לחתוך עם `grep`, עם pandas או עם כל כלי אחר",
        "Dashboard חי בכתובת `/costs/llm` מתרענן כל 60 שניות — pivot לפי סוכן, לפי מודל, לפי tier, או לפי תאריך",
        "4 שכבות עלות (tiers): `ollama-local` (חינם לגמרי, רץ מקומית על [Ollama](/guide/ollama)), `gemini-flash` (חינם עד 1M tokens בחודש), `stepfun-openrouter` (מודל StepFun בחינם דרך OpenRouter), `claude-sonnet` (בתשלום — 3$ לכל מיליון tokens נכנסים, 15$ יוצאים)",
        "אצלי: העלות בפועל של [Claude Sonnet](/claude-code) יוצאת בסביבות 2-5$ בחודש, כי הוא משמש רק כ-fallback חירום. רוב העבודה נעשית ב-Gemini Flash החינמי",
      ],
      tips: [
        "הוסיפו מעקב עלויות כבר ביום הראשון של הפרויקט, לפני שיש משהו מה לעקוב אחריו. אחרת מגלים חודש אחרי השקה שמשהו רץ ב-loop ושרף 200$ — ואין שום דרך לדעת איזה endpoint אחראי לזה",
      ],
    },
    {
      id: "llm-route",
      ...
      subtitle: "endpoint אחד, 4 שכבות fallback אוטומטיות, 0$ ברוב הזמן",
      description:
        "במקום שכל סוכן יחליט בעצמו איזה מודל LLM לקרוא (ומתי ליפול בחזרה למודל אחר אם יש כשל), Delegator חושף endpoint אחד בשם `/llm/route`. הסוכן שולח את הפרומפט עם system prompt ומגבלת tokens, ו-Delegator מחליט בעצמו: מנסה קודם את המודל הזול ביותר (חינמי), אם נכשל עובר לבא בתור, וכך הלאה עד שמשהו עובד. התשובה כוללת גם איזה tier ואיזה model בפועל ענו, כדי שהסוכן ידע.",
      color: "from-pink-600 to-rose-500",
      difficulty: "advanced",
      content: [
        "שכבה 1 — Ollama Local: אם [Ollama](/guide/ollama) (סביבת הרצה של מודלים מקומית) רץ על אותו שרת עם המודל qwen3:4b טעון — הקריאה חינמית לחלוטין, והכי מהירה. דורש זיכרון RAM של 8GB+ ורצוי GPU",
        "שכבה 2 — Gemini 2.5 Flash: חינמי עד 1 מיליון tokens לחודש דרך Google AI Studio. איכות מצוינת לרוב המשימות, כולל עברית",
        "שכבה 3 — StepFun דרך OpenRouter: מודל סיני חזק עם suffix של `:free` ב-OpenRouter. חינם לגמרי בלי תקרת tokens, אבל יש rate limit",
        "שכבה 4 — Claude Sonnet: בתשלום, משמש רק אם כל הקודמים נכשלו. הגדרתי אותו כ-fallback חירום בלבד",
    ...
    },
    {
      ...
      difficulty: "advanced",
      content: [
        "לא להשתמש ב-framework זה יתרון — אבל דורש לבנות את ה-middleware בעצמכם: logging, CORS (הרשאות דפדפן בין דומיינים), auth. לוקח יום, אבל אחר כך אתם בשליטה מלאה",
        ...
        "גיבוי: בכל patch משמעותי אני שומר עותק עם timestamp (`delegator.py.bak-sprint-6.7`) — ככה אם שבירה — rollback זה שינוי של שם קובץ אחד",
      ],
      tips: [
