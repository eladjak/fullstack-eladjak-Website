...
export const delegatorGuide: AgentGuideData = {
  ...
  sections: [
    ...
    },
    {
      ...
      beginner:
        "endpoint זה פשוט כתובת שיודעת לעשות דבר אחד, כמו מספר טלפון של מחלקה ספציפית בחברה. אחד שולח מייל, אחד בודק את היומן, אחד מפרסם פוסט. אצלי (אלעד) ל-Delegator יש יותר ממאה כאלה, מסודרים לפי תחומים: פרסום, AI, גוגל, ניטור. אבל הנקודה החשובה היא שזה לא רק 'צינור' שמעביר בקשות הלאה, בתוך כל כתובת יש גם היגיון אמיתי, כמו שומר שבודק אם מותר לפרסם עכשיו, או מנגנון שבוחר את ה-AI הזול ביותר שמתאים למשימה.",
      content: [
    ...
    },
    {
      ...
      difficulty: "intermediate",
      beginner:
        "דמיינו בניין שבו כל הדואר עובר דרך חדר מיון אחד בכניסה. הרווח הוא עצום: אם פתאום רוצים לבדוק כל מכתב, או להגביל כמה מכתבים כל מחלקה שולחת ביום, מספיק לשנות דבר אחד בחדר המיון, וזה חל על כולם מיד. זה בדיוק רעיון ה-Gateway: במקום שכל סוכן ידבר ישירות עם העולם, כולם עוברים דרך שער אחד. וכך, להוסיף תיעוד, הגבלת קצב או ניסיון חוזר אוטומטי כשמשהו נכשל, זה שינוי במקום אחד, לא בעשרה.",
      content: [
    ...
    },
    {
      ...
      difficulty: "intermediate",
      beginner:
        "כל פעם שסוכן 'חושב' הוא בעצם שולח שאלה למודל AI, וזה עולה כסף, קצת בכל פעם, אבל זה מצטבר. בלי מעקב, אפשר לגלות בסוף החודש חשבון מפחיד בלי לדעת בכלל מאיפה הוא בא. לכן Delegator סופר כל קריאה כזאת: מי שאל, איזה מודל ענה, וכמה זה עלה. אצלי (אלעד) זה מה שמאפשר לראות בלחיצה ש'סוכן אחד אכל 40% מהתקציב החודשי', ולתפוס מיד אם משהו רץ בלולאה ושורף כסף. הכלל שלי: להוסיף מעקב עלויות ביום הראשון, עוד לפני שיש מה למדוד.",
      content: [
    ...
    },
    {
      ...
      difficulty: "advanced",
      beginner:
        "זה אולי הרעיון הכי חכם ב-Delegator, וקל להבין אותו: כשסוכן צריך 'חשיבה' של AI, הוא לא בוחר בעצמו לאיזה מודל לפנות. במקום זה הוא מבקש מ-Delegator, וזה מנסה קודם את הזול ביותר (חינמי לגמרי), ורק אם הוא נכשל או עמוס, עובר לבא בתור, וכך הלאה עד שמשהו עונה. כמו מים שזורמים תמיד למסלול הזול ביותר. אצלי (אלעד) התוצאה היא שרוב העבודה נעשית בחינם, והמודל היקר (Claude) נכנס רק כגלגל הצלה במקרי חירום, ולכן הוא עולה לי בערך 2-5 דולר בחודש בלבד.",
      content: [
    ...
    },
    {
      ...
      difficulty: "advanced",
      beginner:
        "הסעיף הזה הוא לא תיאוריה, כל טיפ כאן נולד מטעות שעשיתי בדרך. אם להוציא מכל זה משפט אחד למי שלא מהתחום, הוא זה: בנו דברים שמתאוששים לבד. השרת מוגדר כך שאם הוא קורס מסיבה כלשהי, הוא קם בחזרה תוך חמש שניות בלי שאגע בו; כל גרסה נשמרת לפני שינוי כדי שאפשר יהיה לחזור אחורה בשנייה; והכול חשוף לעולם דרך מנהרה מאובטחת בלי לפתוח שום פתח מסוכן בשרת. אלה הדברים הקטנים שהופכים מערכת 'שעובדת' למערכת שאפשר לסמוך עליה בלילה.",
      content: [
