  ...
  sections: [
    ...
    },
    {
      ...
      content: [
        ...
        "4 שכבות עלות (tiers): `ollama-local` (חינם לגמרי, רץ מקומית על [Ollama](/guide/ollama)), `gemini-flash` (שכבה חינמית של Gemini 2.5 Flash — ~15 RPM ומכסת tokens יומית נדיבה דרך Google AI Studio), `stepfun-openrouter` (מודל StepFun עם suffix `:free` ב-OpenRouter — מוגבל ב-~50 בקשות ליום בלי קרדיט, או 1000/יום עם $10 קרדיט), `claude-sonnet` (Claude Sonnet 4.6 בתשלום — 3$ לכל מיליון tokens נכנסים, 15$ יוצאים)",
        "אצלי: העלות בפועל של [Claude Sonnet 4.6](/claude-code) יוצאת בסביבות 2-5$ בחודש, כי הוא משמש רק כ-fallback חירום. רוב העבודה נעשית ב-Gemini Flash החינמי",
      ],
      tips: [
    ...
    },
    {
      ...
      content: [
        ...
        "שכבה 3 — StepFun דרך OpenRouter: מודל סיני חזק עם suffix של `:free` ב-OpenRouter. השכבה החינמית מוגבלת ל-~50 בקשות ליום (או ~1000/יום אם יש לכם $10 קרדיט ב-OpenRouter). שימושי כ-fallback נוסף",
        "שכבה 4 — Claude Sonnet 4.6: בתשלום, משמש רק אם כל הקודמים נכשלו. הגדרתי אותו כ-fallback חירום בלבד. אופציונלית אפשר גם להוסיף Haiku 4.5 (זול יותר) או Opus 4.7 (לראסונינג כבד)",
