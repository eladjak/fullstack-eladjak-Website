...
> **Update (July 2026):** this post describes the original 3-agent architecture from March 2026, and is kept as-is as a snapshot of that stage. The network has since grown to 12 agents and migrated from the two Hetzner instances to a single Contabo server (16 vCPU / 62GB), with an orchestrator brain and an output guardian on top. The up-to-date architecture is documented in the [agent guides](/guide).
...
> **עדכון (יולי 2026):** הפוסט מתאר את ארכיטקטורת 3 הסוכנים המקורית ממרץ 2026, ונשמר כתיעוד של אותו שלב. מאז הרשת גדלה ל-12 סוכנים ועברה משני שרתי Hetzner לשרת Contabo אחד (62GB), עם מוח-תזמור ושומר-תוצר מעליה. הארכיטקטורה העדכנית מתועדת ב[מדריכי הסוכנים](/guide).
