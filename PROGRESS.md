...
- **Root cause found:** an intermediate "polish" commit on this branch had rewritten the ENTIRE `:root` + `.dark` token blocks in `src/styles/globals.css` from the site's purple/amber to a rose scale (with a "canonical brand is rose" rationale that is WRONG for this site — rose is Elad's *personal* brand, per `web-design-stack.md`; this dev-portfolio's brand is purple/amber). Because Tailwind maps `primary`/`accent`/`secondary`/`ring`/`chart-*` all to those CSS vars, that one rewrite cascaded rose across every `text-primary`/`from-primary` surface site-wide — including the journey captions, eyebrows, glass cards, glows and the audio-toggle.
- **CSS fix (`globals.css`):** restored `:root` (light) + `.dark` token blocks to the DESIGN.md purple/amber values (kept the deepened near-black cinematic base as `#09090B` Zinc-950). Converted every remaining rose HSL (hue 331/315) → brand violet (hue 258/252) in: `.dark body` ambient wash, `::selection`, `.glow-border`, `.text-glow`, `.cj-root` base, `.cj-scrim` bloom, `.cj-caption-card` shadow + hairline, `.cj-eyebrow::after` underline. Neutralised the rose-tinted caption-card charcoals to cool violet-charcoal.
- **Component fix:** `cinematic-audio.tsx` hover-glow `hsla(331…)` → `hsla(258…)`; comment in `cinematic-journey.tsx` ("rose glow" → "violet glow"). `icon.svg` favicon `#E11D74` → `#A78BFA`. (`cinematic-scene-caption.tsx` + the two section files already used token-based `text-primary`, so they flipped to purple automatically.)
- **Scene stills + videos re-graded (the big one):** all 6 poster `.webp` + 6 source `.jpg` stills + 12 `.mp4` legs (desktop + mobile) were generated rose/magenta. A global hue-rotation drags the neutral space tones into GREEN, so instead used a **luminance→violet DUOTONE** (`scripts/regrade-cinematic-violet.mjs`, ffmpeg: `format=gray` → per-channel curves onto a black→`#8B5CF6`→white ramp → sat/contrast lift). Result: rich cinematic depth (deep violet blacks, purple mids, white cores), zero green, squarely on brand — NOT a flat/AI-slop purple wash. Rose originals backed up to `public/videos/cinematic/_rose-originals/` (gitignored) for rollback; script is idempotent (re-grades from the backup, never compounds). No fresh gpt-image-2/fal regeneration was needed — the duotone preserved every scene's composition and quality.
- **Zero rose proof:** grep of all cinematic surfaces + `globals.css` for rose HSL(300-359)/rose hex → the only remaining hits are my own explanatory comments (the applied `#E11D74`/hue-331 values are all gone). Live Playwright pass (desktop + mobile, 6 scenes + reduced-motion) confirms the running site reads purple with the amber CTA.
- **Gates:** `tsc --noEmit` 0 · `next build` 0 (100 static pages, 28 routes) · live server 200 + Playwright scene screenshots verified purple/amber.
- **Next phase (NOT started):** this luminance-duotone + token-restore pattern, and the whole scroll-narrative journey format, are reusable to bring the cinematic scroll-story to Elad's OTHER sites — each on ITS OWN brand palette (never rose-by-default). Awaiting Elad's go on the preview before touching any other site.
